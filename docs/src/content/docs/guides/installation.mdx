...
<Tabs>
  ...
  <TabItem label="macOS">
    ...
    ```sh
    launchctl print gui/$(id -u)/bot.peppy.agent
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label="macOS">
    Start the service:
    ```sh
    launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/bot.peppy.agent.plist
    ```

    Stop the service:
    ```sh
    launchctl bootout gui/$(id -u)/bot.peppy.agent
    ```

    Restart the service:
    ```sh
    launchctl kickstart -k gui/$(id -u)/bot.peppy.agent
    ```
  </TabItem>
</Tabs>
