...
```  
  ...
  <TabItem label="Rust">
```sh
peppy node add https://github.com/Peppy-bot/example_nodes.git/rust/fake_uvc_camera
```
or
```sh
peppy node add --ref main https://github.com/Peppy-bot/example_nodes.git/rust/fake_uvc_camera
```
<Aside type="caution">
...
<Tabs>
  ...
  ```bash
  sudo apt update
  sudo apt install pkg-config ffmpeg libavdevice-dev libavcodec-dev libavformat-dev libavutil-dev libswscale-dev libdav1d-dev libswresample-dev libavfilter-dev clang libclang-dev llvm-dev
  ```
  </TabItem>
  <TabItem label="macOS">
  ```bash
