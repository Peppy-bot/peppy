...
As an example, we'll try to pull the `fake_uvc_camera` node from [this repo](https://github.com/Peppy-bot/example_nodes) in a separate shell.
...
<Aside type="caution">
The build may fail here if you don't have the required `ffmpeg` libraries installed on your system. 
To install them, run:
...
<Tabs>
  ...
  ```bash
  sudo apt update
  sudo apt install ffmpeg libavdevice-dev libavcodec-dev libavformat-dev libavutil-dev libswscale-dev libdav1d-dev libswresample-dev libavfilter-dev clang libclang-dev llvm-dev
  ```
  </TabItem>
  <TabItem label="macOS">
  ```bash
...
After installing, attempt to add the node again.
</Aside>
