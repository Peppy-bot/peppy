...
As an example, we'll try to pull the `fake_uvc_camera` node from [this repo](https://github.com/Peppy-bot/example_nodes).
...
And if we look at the logs of the started `fake_video_reconstruction` node:
```text
...
In the same way you can share your own nodes by hosting them in a git repository and/or plug them with remote nodes.
