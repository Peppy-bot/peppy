...
We see that video frames are emitted from that node. We can pull a new node that depends on `fake_uvc_camera` to read those frames and reconstruct a short video.
