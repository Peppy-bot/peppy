...
A slot left without a `--bind` is legal but stays silent: the receiver would boot and simply never yield a message.
...
The running receiver is still bound to `gifted-moser-9365` only, so it will not see the new producer. To receive from both, restart the receiver with one `--bind` per producer (repeating `--bind` on the same slot accumulates producers on it):
```sh
❯ peppy node stop vigorous-buck-8117
❯ peppy node run hello_receiver:v1 \
    --bind hello_world_param@gifted-moser-9365 \
...
│                                      │                              │         │         │ gifted-moser-9365@core-no │
│                                      │                              │         │         │ de-sweet-germain-4388,    │
...
The `Instance bindings` table makes the routing explicit: `hello_receiver`'s instance resolved its `hello_world_param` slot to the two producers we bound, each rendered as `instance_id@core_node` (its full wire address). A slot only ever receives from the producers bound to it: that is exactly why `vigorous-buck-8117` prints messages from both `gifted-moser-9365` and `admiring-black-0614` above, and why an unbound slot renders `(unbound)` and stays silent.
