...
Here we have `interfaces` organized by kind (`topics`, `services`, `actions`). Topics use `emits` and `expects` lists, while services and actions use `exposes` and `consumes` lists. These interfaces define the dependencies between nodes.
