...
Here we have `interfaces` organized by kind (`topics`, `services`, `actions`), each containing `exposes` and `consumes` lists. These interfaces define the dependencies between nodes.
