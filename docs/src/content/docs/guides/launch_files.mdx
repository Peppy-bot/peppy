...
The output confirms that all nodes have been added to the stack along with their dependency relationships. The `Instance bindings` table breaks that down per instance: the consumers (`fake_robot_brain`, `fake_video_reconstruction`) list the `depends_on` slots they resolved, while the producers (`fake_openarm01_controller`, `fake_uvc_camera`) and the core node declare no slots and show `(none)`. A resolved binding shows the chosen producer(s) as `instance_id@core_node`, ordered by link ID; a multi-cardinality slot lists every bound member in binding order, and a `zero_or_more` slot bound to nothing shows `(empty set)`. Every declared producer slot (`depends_on.nodes` / `depends_on.contracts`) except `zero_or_more` must be bound; a launcher that leaves one out is rejected at validation (see [dependency cardinality](/advanced_guides/topics/#dependency-cardinality)). Pairing and observer slots follow their own rules: a participant slot is paired or declared [`vacant`](#links-optional), and an observer slot is linked or declared vacant unless its cardinality is `zero_or_more`, where omitting it IS its empty set. The launch also prints a `Slice of launch ...` line above the tables naming the launch that produced this stack and its coordinator.
...
A slot that starts unresolved on purpose says so here too, as `{ vacant: "<why>" }`. That value binds nothing and covers the slot, so the fate of every declared slot is one entry in this one map and a slot you forgot is an absence rather than a value. It is legal exactly where the slot has no other way to write "empty": a participant pairing slot, and a `cardinality: "one"` observer slot. A multi-cardinality slot writes its empty set as `[]` (or omits the key), and a required producer slot has no empty state at all; a vacancy on any of those is rejected, naming the spelling that slot does have. The reason is required, is trimmed, and travels with the instance, so `peppy stack launch` reports it back when the node starts.
  ...
  instances: [
    {
      ...
      links: {
        ...
        arm: "arm_1/limb",
        // A `one` observer slot names a single source instance to observe.
        // The multi cardinalities take these same values in an array.
        watch: "ctrl_1",
        // A slot this deployment leaves empty on purpose, and why.
...
Two instances of the same node can choose different fates for the same slot, which is why this is written per instance rather than in the node's manifest:
