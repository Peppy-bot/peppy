...
The output confirms that all nodes have been added to the stack along with their dependency relationships. The `Instance bindings` table breaks that down per instance: the consumers (`fake_robot_brain`, `fake_video_reconstruction`) list the `depends_on` slots they resolved, while the producers (`fake_openarm01_controller`, `fake_uvc_camera`) and the core node declare no slots and show `(none)`. A resolved binding shows the chosen producer(s) as `instance_id@core_node`, ordered by link ID; a multi-cardinality slot lists every bound member in binding order, and a `zero_or_more` slot bound to nothing shows `(empty set)`. Every declared producer slot (`depends_on.nodes` / `depends_on.contracts`) except `zero_or_more` must be bound; a launcher that leaves one out is rejected at validation (see [dependency cardinality](/advanced_guides/topics/#dependency-cardinality)). Pairing and observer slots follow their own rules: a participant slot is paired or, where its manifest declares it `optional: true`, declared [`vacant`](#links-optional); an observer slot is linked or, where its manifest declares it `cardinality: "zero_or_one"`, declared vacant, unless its cardinality is `zero_or_more`, where omitting it IS its empty set. The launch also prints a `Slice of launch ...` line above the tables naming the launch that produced this stack and its coordinator.
...
Vacancy is a two-sided contract, and the node owns one side of it. A slot is vacatable only where the node's own manifest declares it emptiable: `optional: true` on a `depends_on.pairings` entry, or `cardinality: "zero_or_one"` on a `depends_on.pairing_observers` entry. On a slot the manifest declares required, a vacancy is rejected and the error names the manifest key that would lift the bar. Every other slot is rejected with the spelling it does have: a `zero_or_more` slot, producer or observer, writes its empty set as `[]` (or omits the key), and a `one` / `one_or_more` producer slot and a `one_or_more` observer slot have no empty state at all. The manifest says "may be empty", the launcher says "is empty here, because", and forgetting the slot entirely stays an error whichever the manifest says.
