...
Every node build a launch performs goes through the [build artifact cache](/advanced_guides/containers/#build-caching): a deployment whose staged sources are byte-identical to an earlier build's reuses that build's artifact, so launching the same launcher again, after a daemon restart or on another machine booted from the same disk, spends seconds on the build phase instead of rebuilding every node. Each reuse is reported in the launch output and in that node's build log:
