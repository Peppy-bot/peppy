...
[`examples/rust_robot.json5`](https://github.com/Peppy-bot/launchers-hub/blob/main/examples/rust_robot.json5)
 ...
 3. Each deployment requires a `source` (where the node comes from) and an `instances` array (how many copies to start, and how each one is configured). Every entry needs an `instance_id`; `arguments`, `env_vars`, `framework`, `core_node`, `links`, and `defer_links` are optional. See [Instance attributes](#instance-attributes) for what each one does
...
The output confirms that all nodes have been added to the stack along with their dependency relationships. The `Instance bindings` table breaks that down per instance: the consumers (`fake_robot_brain`, `fake_video_reconstruction`) list the `depends_on` slots they resolved, while the producers (`fake_openarm01_controller`, `fake_uvc_camera`) and the core node declare no slots and show `(none)`. A resolved binding shows the chosen producer(s) as `instance_id@core_node`, ordered by link ID; a multi-cardinality slot lists every bound member in binding order, and a `zero_or_more` slot bound to nothing shows `(empty set)`. Every declared producer slot (`depends_on.nodes` / `depends_on.contracts`) except `zero_or_more` must be bound; a launcher that leaves one out is rejected at validation (see [dependency cardinality](/advanced_guides/topics/#dependency-cardinality)). Pairing and observer slots follow their own rules: a participant slot is paired or covered by [`defer_links`](#defer_links-optional), and an observer slot is linked or deferred unless its cardinality is `zero_or_more`, where omitting it IS its empty set. The launch also prints a `Slice of launch ...` line above the tables naming the launch that produced this stack and its coordinator.
...
One instance filling all three kinds:
...
The `link_id`s of pairing or observer slots this instance should start with left unresolved, listed by name. Every participant slot must either be paired or appear here, and every `one` / `one_or_more` observer slot must either be linked or appear here; a producer-binding slot cannot be deferred. A `zero_or_more` observer slot needs no entry (omitting it is its empty set), though deferring it is valid and says the same thing explicitly. Deferring a slot the same plan already links is an error. Use it when the peer arrives later, at runtime, rather than in this launch.
