...
Each instance can optionally include a `variant` field to pick which
variant of the node it runs. Placing `variant` on the instance (rather
than on the source) lets a single deployment block bring up multiple
variants of the same `(name, tag)` side by side: each unique variant
becomes a distinct entity in the stack, and consumers receive messages
from every running variant.

The `variant` object supports three formats:

 - **Name**: Reference a variant declared in the node's manifest — `{ name: "mock-rust" }`
...
```json5
// Named variant on a single instance
{
  source: {
...
```json5
// Git variant — fetch the variant directly from a git repository
{
...
```json5
// URL variant — fetch the variant from an HTTP archive
{
  source: { local: "./my_node" },
  instances: [
    {
...
```json5
// Multiple variants of the same node, one deployment block
{
  source: { name: "depth_camera:0.1.0" },
  instances: [
    {
      instance_id: "torso_front_camera",
      variant: { name: "realsense_d435" }
    },
    {
      instance_id: "left_arm_camera",
      variant: { name: "realsense_d405" }
    },
    {
      instance_id: "right_arm_camera",
      variant: { name: "realsense_d405" }
    }
  ]
}
...
The launcher fans this out into one `(name, tag, variant)` entity per
unique variant: in the example above, `depth_camera:0.1.0@realsense_d435`
runs alongside `depth_camera:0.1.0@realsense_d405`, with the two
`realsense_d405` instances assigned to the same entity.
...
│ fake_openarm01_controller:0.1.0         │ Ready │ default │ 1 running  │ /Users/tuatini/.peppy/built_nodes/fake_openarm01_controller_0.1.0/default.tar.zst │
│ fake_robot_brain:0.1.0                  │ Ready │ default │ 1 running  │ /Users/tuatini/.peppy/built_nodes/fake_robot_brain_0.1.0/default.tar.zst          │
│ fake_uvc_camera:0.1.0                   │ Ready │ default │ 2 running  │ /Users/tuatini/.peppy/built_nodes/fake_uvc_camera_0.1.0/default.tar.zst           │
│ fake_video_reconstruction:0.1.0         │ Ready │ default │ 1 running  │ /Users/tuatini/.peppy/built_nodes/fake_video_reconstruction_0.1.0/default.tar.zst │
└─────────────────────────────────────────┴───────┴─────────┴────────────┴──────────────────────────────────────────────────────────────────────────────────┘
