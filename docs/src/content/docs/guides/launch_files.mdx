...
The `source` attribute supports four formats:
 ...
 - **Repository**: Reference a node that is already registered in your user repositories. Specify `name` and `tag`, or use the combined `name: "<name>:<tag>"` shorthand. The node is resolved against your local packages cache at `~/.peppy/cache/nodes.json5`.
