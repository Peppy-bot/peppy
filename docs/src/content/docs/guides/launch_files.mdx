...
[`examples/rust_robot.json5`](https://github.com/Peppy-bot/launchers_hub/blob/main/examples/rust_robot.json5)
 ...
 3. Each deployment requires a `source` and `instances` attribute. Besides `instance_id`, an instance can carry `arguments`, `env_vars`, `bindings` (pinning `depends_on` slots to producer instances), and — for nodes declaring [pairing](/advanced_guides/pairing) slots — `pairings` (pairing a slot with a peer instance at launch) and `defer_pairings` (explicitly starting a required slot unpaired)
