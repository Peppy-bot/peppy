...
| `with_bound_producer` | one `--bind` occurrence, or one launcher `links:` entry, for a `depends_on.nodes` / `depends_on.contracts` slot |
...
Repeat calls for the same `link_id` accumulate in call order, exactly as repeated `--bind` and `--link` flags do, so the Nth call is the Nth member the node reads back.
