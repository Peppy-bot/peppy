...
2. If the instance doesn't exit cooperatively in time (the grace window plus the brief teardown its runtime needs afterwards), the daemon **force-kills its whole process group** (the node and any child processes it spawned), so nothing is left running.
