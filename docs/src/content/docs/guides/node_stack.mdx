...
With no arguments, `stack list` discovers every live core node in the same messaging namespace and prints one section per distinct core-node name: the local name first, then the others by name. If multiple live daemons collide on one name, discovery deduplicates them into a single warning-bearing section; its header identifies the name and annotates it with the answering daemon's hostname. Use `peppy stack list --core-node <name>` to filter the output to that one name. If a core node disappears between discovery and the stack query, the other sections still render, the failed section contains an error line, and the command exits non-zero.
