...
Below the node table, the **`Instance bindings`** table expands the stack one level further: one row per instance, grouped by node, alongside the slot **bindings** that instance resolved when it started. A binding ties one of the consumer's `depends_on` slots (identified by its `link_id`) to the producer instance(s) that feed it, rendered as `link_id → producer`, each producer shown as `instance_id@core_node` and comma-separated when a `from_any` slot is bound to several. A `from_any` slot left unbound resolves to `link_id → (unbound)`: the slot is silent — it receives from no producer until a launch binds it. An instance whose node declares no `depends_on` shows `(none)`. Only nodes that currently have at least one instance appear in this table. See [Bindings and routing](/advanced_guides/topics/#bindings-and-routing) for how slots are declared and how `--bind` fills them. In the listing above neither node declares a dependency, so every binding cell is `(none)`.
