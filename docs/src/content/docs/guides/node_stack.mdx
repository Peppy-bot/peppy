...
This command runs the `add_cmd` from the node's `peppy.json5` process configuration and registers it with the daemon node to make it available for execution.
