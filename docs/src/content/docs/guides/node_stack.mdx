...
This command registers the node with the core node by staging a snapshot of the source directory. By default, `peppy node add` does **not** run `build_cmd` — it only adds the node to the stack in the `Added` stage. To also run the build, use `peppy node add . --build` or run `peppy node build` separately afterward.
