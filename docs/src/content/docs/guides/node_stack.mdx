...
:::caution
`peppy node add` does **not** run the node's `build_cmd`. To produce a runnable artifact you either run `peppy node build <name>:<tag>` afterwards, or chain the two in one shot with `peppy node add . --build` (shorthand `-b`). Adding `--run` (`-r`) also spawns an instance once the build finishes, and implies `--build`.
:::
