...
If you pass runtime `key=value` arguments alongside `-r`, keep them **after** the source path (e.g. `peppy node add -sr . frequency=30`), so the source positional isn't confused with the trailing args.
:::
