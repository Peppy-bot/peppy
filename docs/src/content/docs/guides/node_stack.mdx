...
This command runs the `add_cmd` from the node's `peppy.json5` build configuration and registers it with the daemon node to make it available for execution.
...
```
This will run the `start_cmd` command from the `peppy.json5` build configuration.
