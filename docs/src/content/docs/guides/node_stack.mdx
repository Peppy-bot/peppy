...
Run-time flags (`--bind KEY@VALUE`, `--instance-id`) and trailing `key=value` arguments are only accepted when `--run` is set; without `--run` there is no instance to apply them to, and the CLI rejects the combination at parse time.
:::
