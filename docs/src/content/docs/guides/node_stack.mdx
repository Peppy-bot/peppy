...
With no arguments, `stack list` discovers every live core node in the same messaging namespace and prints a separate section for each daemon: the local daemon first, then the others by core-node name. The section header identifies the core node and annotates it with that daemon's hostname. Use `peppy stack list --core-node <name>` when you want exactly one remote daemon's section. If a daemon disappears between discovery and the stack query, the other sections still render, the failed section contains an error line, and the command exits non-zero.
