...
Error: required pairing slot(s) not covered: [arm]. Pass `--link <link_id>@<peer_instance>`
to pair each at start, or `--defer-link <link_id>` to explicitly start unpaired
```
...
Declare an observer slot in `depends_on.pairings` with `observes_role` instead of `role`, and list the topics it taps in `interfaces.topics.consumes` under its `link_id`. An observer plays no role, claims no endpoint, and emits nothing, so it never needs coverage on an emit side:
