...
The generated modules live alongside participant slots under `peppygen.paired_topics.<link_id>.<topic>`. Every observer slot is required (it has no `optional` form); cover it with a `--link`/`links:` entry or defer it explicitly with `--defer-link`/`defer_links:`, exactly like a required pairing slot.
