...
After `peppy repo refresh`, the pairing is cached and addressable by `(name, tag)`. `peppy repo search <name>:<tag>` lists the indexed nodes that play its roles and the ones that observe them (see [Search the repositories](/advanced_guides/repositories/#search-the-repositories)).
