...
**Pairing** is PeppyOS's first-class mechanism for the cases where two nodes genuinely talk to each other. A *pairing* is a named, versioned contract with exactly **two roles**; two node **instances** — one per role — are *paired* 1:1 over it, and each side's topics flow only to its paired peer. The mental model is Bluetooth:
...
A pairing dependency is deliberately **invisible to the node dependency graph**: two nodes paired with each other never form a cycle, and neither requires the other to be present at build time. Pairings are **topics only**; see [Why topics only?](#why-topics-only).
...
A pairing is a standalone `pairing/v1` document in a repository peppy scans (see [Repositories](/advanced_guides/repositories)). Like an interface, the `message_format` lives here once and both sides inherit it by reference — but unlike an interface, a pairing names *both* directions and the two roles playing them.
...
The generated code addresses each arm through its slot module — `peers::left_arm::joint_commands` vs `peers::right_arm::joint_commands` — and each subscription receives only its own arm's states. When a peer instance declares several complementary slots (pairing two commanders' arms to each other, say), disambiguate the target with `--pair left_arm@cmd_2/right_arm`.

## Why topics only?

PeppyOS has three communication patterns (topics, services, and actions), but a pairing may declare **topics only**. The reason is the distinction that shapes the rest of the dependency model: a topic dependency is passive, while service and action dependencies are caller-driven.
...
## Pairing vs. interfaces (`from_any`)

Pairing and [interface conformance](/advanced_guides/interface_conformance) both decouple nodes from each other through a shared contract. They answer different questions:

| | Pairing | Interface (`from_any`) |
|---|---|---|
| Cardinality | Exactly 1:1 per slot, exclusive | Any number of conforming producers |
| Directionality | Both directions in one contract (two roles) | One direction per interface |
| Establishment | Explicit (`--pair` / `pairings:`), at instance start | Implicit: any conforming producer that publishes is received |
...
Use a **pairing** when the relationship is exclusive and both directions belong to one conversation — a controller and *its* arm. Use **interfaces** when any number of loosely-coupled producers should feed a consumer — every arm on the floor publishing diagnostics to one observer. The two compose: `robot_arm` can be paired with its controller *and* conform to a one-way `joint_state_source` interface so dashboards can watch it.
