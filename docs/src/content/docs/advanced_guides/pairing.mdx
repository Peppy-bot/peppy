...
- **Read-only.** The generated module for an observer topic exposes the same subscription surface as a consumed topic, plus a `source()` helper reporting the resolved source (or `None` before it resolves). There is no publisher and no `paired()`; an observer never talks back.
- **Not shown in `peppy stack list`.** The `Instance pairings` table renders participant slots only; inspect an observer's link through its generated `source()` helper instead.

The generated modules live alongside participant slots under `peppygen.paired_topics.<link_id>.<topic>` (Python) / `peppygen::paired_topics::<link_id>::<topic>` (Rust). Every observer slot is required (it has no `optional` form); cover it with a `--link`/`links:` entry or defer it explicitly with `--defer-link`/`defer_links:`, exactly like a required pairing slot.
