...
- **Incarnation-isolated.** A delivered pin carries the peer's incarnation, the daemon-allocated number every run publishes under as the trailing segment of its topic key expressions, and the slot's wire subscription pins it. A restarted peer is re-paired under a strictly newer incarnation, so a message from its previous run differs on a literal key segment and can never reach the re-pinned subscription.
...
- **Incarnation-isolated.** Every run of an instance publishes under a daemon-allocated incarnation number, carried as the trailing segment of its topic key expressions, and an observer's wire subscription pins the incarnation named in its member pin. Cross-run leakage is therefore structurally impossible: a message from a previous run of the source differs from the current run on a literal key segment and can never match a subscription pinned to the new incarnation. When the source restarts, the observer hears a clean gap until the daemon's lifecycle notify delivers the new incarnation and the subscription redeclares onto the new run's key; the gap spans the notify latency, and nothing from the old run can fill it.
...
# `watch` is one_or_more: read the whole observed set, in plan order.
...
An observed set is **seeded at spawn and live after**. The boot config the daemon hands the process carries every declared slot's planned member set, stamped with each source's incarnation and liveness at that moment, so `sources()` answers the deployment's membership from the node's first instruction, setup included: a node can discover the robot's shape before it starts serving. After that the set is live, which is the one way it differs from a bound producer set: the daemon replaces it whole whenever the plan's observed pairings change, so `sources()` can return a different set between reads, and an empty read means the plan observes nothing through that slot. A member whose source is down (or has not started yet) stays in the set, at its position, so the slot's shape does not flicker with the fleet's health.
