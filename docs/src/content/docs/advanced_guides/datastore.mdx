...
- **Keys are any non-empty string.** A key is carried in the request payload, not as a Zenoh keyexpr, so any character is allowed: slashes, spaces, dots, wildcards such as `*` and `{}`, and Unicode all round-trip unchanged. The only rejected key is the empty string, refused before any request is sent. So `robot_state_1`, `robot/state.last?v=2`, and `日本語` are all valid keys, while `""` is not.
