...
- **Keys are arbitrary non-empty strings.** A key rides inside the request payload, not as a Zenoh keyexpr, so any character is allowed: slashes, spaces, `*`, `{}`, anything. `"robot/state**{1}"` is a perfectly valid key; only the empty string is rejected, before any request is sent.
