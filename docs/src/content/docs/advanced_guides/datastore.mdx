...
- **Keys use the node-name character set.** A key may contain ASCII letters, digits, `_`, and `-`; slashes, spaces, and wildcards such as `*` and `{}` are rejected, as is the empty string, before any request is sent. So `robot_state_1` is a valid key, while `robot/state**{1}` is not.
