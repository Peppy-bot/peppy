...
`peppylib` exposes four helpers:

- `datastore_store`: upsert a value under a key.
- `datastore_get`: read the value back, or `None` if the key was never set.
- `datastore_list`: list every key's metadata (encoding and last writer), without the value bytes.
- `datastore_remove`: delete a key, reporting whether it existed.
...
The store also records, for each key, the `instance_id` of the node that last wrote it. Both `datastore_get` (on the returned `StoredValue`) and `datastore_list` surface this as `last_modified_by`, so you can tell which node owns a value.
...
They are available from Rust as `peppylib::datastore_store`, `peppylib::datastore_get`, `peppylib::datastore_list`, `peppylib::datastore_remove`, `peppylib::StoredValue`, `peppylib::DatastoreEntry`, and `peppylib::Encoding`, and from Python as `from peppylib import datastore_store, datastore_get, datastore_list, datastore_remove, StoredValue, DatastoreEntry, Encoding`. Like the other core node helpers, they are not re-exported by `peppygen`; import them from `peppylib` directly.

Each takes a `NodeRunner`, the same handle your node receives from `NodeBuilder::new().run(...)` (see the [services guide](/advanced_guides/services/) for a typical setup), and talks to the node's bound core node.
...
`datastore_list` returns the metadata of **every** key currently in the store: one entry per key carrying the `key`, its `encoding` tag, and `last_modified_by` (the `instance_id` of the node that last wrote it). The value bytes are deliberately **not** included — a list stays cheap no matter how large your values are; fetch the bytes for a specific key with `datastore_get`. The order is unspecified, and the result is a point-in-time snapshot (another node may store or remove keys immediately after).
...
<Tabs>
  <TabItem label="Python">
    ...
    from peppygen import NodeBuilder, NodeRunner
    from peppygen.parameters import Parameters
    from peppylib import datastore_list
  ...
  <TabItem label="Rust">
    ...
    use peppygen::{NodeBuilder, Parameters, Result};
    use peppylib::datastore_list;

    fn main() -> Result<()> {
        NodeBuilder::new().run(|_args: Parameters, node_runner| async move {
            for entry in datastore_list(&node_runner, Duration::from_secs(3)).await? {
                println!(
                    "{} ({}) last written by {}",
...
`datastore_remove` deletes (unsets) `key` from the store. It returns a boolean: `true` if the key existed and was removed, `false` if it was already absent — so removing a missing key is a no-op, not an error.
...
<Tabs>
  <TabItem label="Python">
    ...
    from peppygen import NodeBuilder, NodeRunner
    from peppygen.parameters import Parameters
    from peppylib import datastore_remove


    async def setup(_params: Parameters, node_runner: NodeRunner) -> None:
        removed = await datastore_remove(node_runner, "calibration/wrist_offset", 3.0)
  ...
  <TabItem label="Rust">
    ...
    use peppygen::{NodeBuilder, Parameters, Result};
    use peppylib::datastore_remove;

    fn main() -> Result<()> {
        NodeBuilder::new().run(|_args: Parameters, node_runner| async move {
            let key = "calibration/wrist_offset";
            if datastore_remove(&node_runner, key, Duration::from_secs(3)).await? {
                println!("removed {key}");
            } else {
...
- **Store is an upsert; last writer wins.** A later store under an existing key overwrites the value, the encoding, and the recorded writer. The store records the storing node's `instance_id` on every write and surfaces it as `last_modified_by` on `datastore_get` and `datastore_list`. There is no atomic read-modify-write, so two nodes that both do get-then-store on the same key can race; design keys so a single writer owns each one when that matters.
- **`datastore_list` is a metadata snapshot.** It returns every key's `encoding` and `last_modified_by` but never the value bytes (fetch those with `datastore_get`), in unspecified order, reflecting the store at the moment it was answered.
