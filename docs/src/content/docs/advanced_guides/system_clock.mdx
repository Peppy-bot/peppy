...
- A daemon-wide flag, `peppy service serve --clock-source=wall|sim`, that decides the default for instances that omit the override.
...
Subscribers see the same shape either way. Until that external publisher lands its first tick, sim mode has no time to serve: `synchronize` fails with an error and a `subscribe` stream yields nothing, so both helpers depend on the publisher being up.
...
For hot-path code that just wants "what time is it now" without caring whether the node was launched in wall or sim mode, peppylib exposes `PeppyClock` (in `peppylib.clock` / `peppylib::clock`) and the generator emits a pre-bound clock module (`peppygen.clock` in Python, `peppygen::clock` in Rust):

- `init(node_runner)`: async; opens a `clock` subscription in sim mode (a no-op in wall mode) so the first `now_ns` after a tick lands returns immediately. Idempotent per node: repeat calls for the same node are no-ops, and initializing a different node rebinds (how consecutive test-harness boots in one process each read their own clock). Call it once at the top of your setup function.
...
Prefer `synchronize` over `PeppyClock` when you need a bounded-staleness offset, since it observes a round trip and reports the RTT. `subscribe` is the right pick if you want every raw tick. For everything else, reach for the generated clock module's `now_ns` (`peppygen.clock.now_ns` in Python, `peppygen::clock::now_ns` in Rust).
