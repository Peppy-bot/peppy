...
- `subscribe_clock` — a long-lived subscription to the periodic `clock` topic the core node publishes. Each `ClockTick` carries the core node's wall clock at emission.
...
| Result | Offset **and** observed RTT | Snapshot of the core node's wall clock |
