...
- `target_variant` — optionally target a specific variant of the exposing node (e.g. `Some("realsense_d435")`), or `None` to match any variant.
- `request` — the request payload.

The response contains:
- `instance_id` — the instance that handled the request.
- `variant` — the variant of the node that handled the request (`"default"` for the baseline).
...
When several variants of the same `name:tag` are running side by side (for example, two `depth_camera:0.1.0` instances, one `realsense_d435` and one `realsense_d405`), `target_variant` lets you steer the call to the implementation you want:

- **`None`** (default) — any variant of the exposing node may answer.
- **`Some("variant_name")`** — only the named variant will answer; pass `"default"` to target the baseline explicitly.

```rust
// Pin the call to the d435 variant
let response = depth_camera_get_depth_frame::poll(
    &node_runner,
    Duration::from_secs(5),
    None,
    None,
...
The response `variant` field tells you which variant actually produced the answer.
