...
A service slot resolves through its binding: the generated `poll` reads the slot's bound producer and sends the wire request directly to it, carrying the producer's full `(core_node, instance_id)` wire address. No discovery is involved. Because a request/response call needs exactly one responder, the slot must be bound to exactly one producer; a slot bound to several fails the call with `ServiceSlotNotPinned` naming the slot (a slot bound to none cannot exist — launch rejects it).
