...
- `core_node`: the core node hosting that consumer instance, read from the request context.
...
For a service without a request body, the handler receives a `Request` with only the request context (`instance_id` and `core_node`):
...
Use `poll` to send a request and wait for a response. The caller selects which bound producer handles the call by passing one explicit `target`, obtained from the slot's [cardinality](/advanced_guides/topics/#dependency-cardinality)-typed accessor: on a `one` slot `bound_producer()` returns the sole producer directly, on a `zero_or_one` slot it returns an `Option` the caller branches on first (a vacant slot has no target, so there is nothing to poll), on a `one_or_more` slot `bound_producers()` returns a never-empty set whose `first()` needs no unwrap, and on a `zero_or_more` slot it returns a possibly empty list whose empty case the caller must handle. The explicit `target` parameter itself has the same shape for every cardinality. The candidates are the slot's bound set when the call is made, every member of it validated by the plan that bound it, so by the time `poll` runs the route set is already validated.
...
The `target` must be a member of the slot's own bound set, the one its bound-producer accessor returns; a target outside the set (including a producer bound to a different slot of the same consumer) fails with `TargetNotBound` (`ValueError` in Python) before anything reaches the wire. A `ProducerRef` yielded by the slot's own topic subscription is a member by construction, so the natural receive-a-frame-then-call-that-camera flow needs no defensive code.

The response contains:
- `instance_id`: the producer instance that handled the request, read from the response context.
- `core_node`: the core node hosting that producer instance, read from the response context.
...
<Tabs>
  <TabItem label="Python">
```python
# Runs once per bound producer, in plan order.
...
<Tabs>
  <TabItem label="Python">
```python
camera = enable_camera.bound_producers(node_runner)[0]
```
  </TabItem>
  <TabItem label="Rust">
```rust
...
```sh
peppy node run --link uvc_camera@my-camera-instance consumer:v1
```
...
3. If the `wrist_right_camera` binding line were removed, validation would reject the launch (a declared slot is bound, or declared vacant where its cardinality is `zero_or_one`): a service call has no wildcard fallback.
...
Service calls can fail with four error types:
- **TargetNotBound** (`ValueError` in Python): the selected target is not a member of the slot's bound set; raised before anything reaches the wire.
