...
Use `poll` to send a request and wait for a response. The caller selects which bound producer handles the call by passing one member of the slot's `bound_producers()` set explicitly. This shape is the same for every [cardinality](/advanced_guides/topics#dependency-cardinality), including the default `one`, where `bound_producers()` contains exactly one member and `first()` always succeeds. The candidates themselves are fixed by the application bindings at launch, so by the time `poll` runs the route set is already validated.
...
async def call_service(node_runner: NodeRunner):
    # `one`: exactly one entry; [0] always succeeds.
    # `one_or_more`: at least one entry; [0] always succeeds.
    # `zero_or_more`: the set may be empty; check before selecting.
    cameras = uvc_camera_enable_camera.bound_producers(node_runner)
    if not cameras:
        print("no producer bound to the `uvc_camera` slot, nothing to enable")
        return

    request = uvc_camera_enable_camera.Request(enable=True)
    response = await uvc_camera_enable_camera.poll(
        node_runner,
        cameras[0],  # which bound instance to reach
        request,
        5.0,  # timeout (seconds)
    )
...
fn main() -> Result<()> {
    NodeBuilder::new().run(|_args: Parameters, node_runner| async move {
        // `one`: exactly one entry; `first()` always succeeds.
        // `one_or_more`: at least one entry; `first()` always succeeds.
        // `zero_or_more`: the set may be empty, so the `else` branch is the
        // normal path when nothing is bound.
        let cameras = uvc_camera_enable_camera::bound_producers(&node_runner);
        let Some(camera) = cameras.first() else {
            println!("no producer bound to the `uvc_camera` slot, nothing to enable");
            return Ok(());
        };
        ...
        let response = uvc_camera_enable_camera::poll(
            &node_runner,
            camera,                 // which bound instance to reach
...
camera = uvc_camera_get_camera_info.bound_producers(node_runner)[0]
...
let cameras = uvc_camera_get_camera_info::bound_producers(&node_runner);
let response = uvc_camera_get_camera_info::poll(
    &node_runner,
    cameras.first().expect("`one` slot binds exactly one producer"),
...
Codegen provides only single-target operations; calling every member of a multi-cardinality slot is a plain loop at the call site. Sequential versus concurrent execution, partial-result collection, and whether one failure cancels the other calls are application decisions; there is no atomic broadcast:

<Tabs>
  <TabItem label="Python">
```python
# Runs once per bound producer.
# `one`: the loop body runs exactly once.
# `one_or_more`: the loop body runs at least once.
# `zero_or_more`: an empty set makes the loop a no-op; no request is sent.
for camera in uvc_camera_enable_camera.bound_producers(node_runner):
    request = uvc_camera_enable_camera.Request(enable=True)
