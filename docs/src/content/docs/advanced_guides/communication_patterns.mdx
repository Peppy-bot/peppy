...
description: Pick between topics, services, actions, pairing, contract conformance, and the datastore with a side-by-side decision guide
sidebar:
  order: 0
---

Peppy gives nodes five ways to exchange data: [topics](/advanced_guides/topics/), [services](/advanced_guides/services/), [actions](/advanced_guides/actions/), [pairing](/advanced_guides/pairing/), and the [datastore](/advanced_guides/datastore/). On top of the first three, [contract conformance](/advanced_guides/contract_conformance/) changes not *what* flows but *who* can be on the other end. Each mechanism has its own guide; this page is the map for picking the right one before you write a manifest.
...
| [Contract dependency](/advanced_guides/contract_conformance/) | `depends_on.contracts` | Any node whose `conforms_to` names the contract | The conforming instances the launcher binds to the slot (one or several) |
...
:::note
A topic slot receives from every producer bound to it and from no one else; every declared slot must be bound (launch rejects unbound slots). For services and actions, every call is answered by exactly one producer: the slot must be bound to a single producer, and the call addresses it directly. See [Why exactly one producer?](/advanced_guides/services/#why-exactly-one-producer).
:::
...
- Use a **contract** when several nodes implement the same contract (a hardware driver and its sim twin, several camera vendors) or when several producers should feed one slot (telemetry fan-in, monitoring, via the launcher's array binding). The consumer names the contract; the launcher picks the producers. See [Pairing vs. contracts](/advanced_guides/pairing/#pairing-vs-contracts) for the boundary on the other side.
...
- **Only caller-driven cycles are cycles.** Topic subscriptions are passive, so two nodes may consume each other's topics; pairing is built on exactly this property. Mutual service or action relationships deadlock, so the daemon rejects them however they are wired, directly or through contracts; see [Caller-driven cycles are rejected](/advanced_guides/contract_conformance/#caller-driven-cycles-are-rejected). When two nodes need both directions, make at least one direction a topic (or model both directions as a pairing), or model the bounded direction as an action from client to server.
...
- `robot_arm` also conforms to a `joint_state_source` **contract**; dashboards consume its state **topic** through a contract slot the launcher binds to every arm on the floor, feeding one monitor without disturbing any pairing.
...
One relationship, one mechanism: the pairing carries the control loop, the action carries the bounded job, the contract carries the fan-in, and the datastore carries the state that outlives any single exchange.
