...
description: Pick between topics, services, actions, pairing, contract implementation, and the datastore with a side-by-side decision guide
sidebar:
  order: 0
---
...
The shape says nothing about which node sits on the other end. A consumer couples to the other end at one of four levels:

| | Declared under | Who can fill the slot | Instances per slot |
|---|---|---|---|
| Direct node dependency | `depends_on.nodes` | Instances of that exact producer node | The producer instance(s) bound to the slot, sized by its cardinality (one by default); a `zero_or_one` slot may also run empty |
| [Contract dependency](/advanced_guides/contract_implementation/) | `depends_on.contracts` | Any node whose `manifest.implements` names the contract | The implementing instance(s) bound to the slot, sized by its cardinality (one by default); a `zero_or_one` slot may also run empty |
| [Pairing slot](/advanced_guides/pairing/) | `depends_on.pairings` | One instance playing the complementary role of the pairing | Exactly one peer at a time, exclusive, both directions; an `optional: true` slot may also run with none |
| [Observer slot](/advanced_guides/pairing/#observers-watching-a-pairing-without-joining-it) | `depends_on.pairing_observers` | Instances playing the observed role of the pairing | The observed pairing(s) bound to the slot, sized by its cardinality (`one` by default, or `zero_or_one` / `one_or_more` / `zero_or_more`), non-exclusive, read-only |

:::note
A node, contract or observer slot takes an application-selected set sized by its declared [cardinality](/advanced_guides/topics/#dependency-cardinality): exactly one by default (`one`), at most one for `zero_or_one`, at least one for `one_or_more`, possibly empty for `zero_or_more`; a launch that leaves a `one` / `one_or_more` slot unbound is rejected. Only participant pairing slots take no `cardinality` key: a pairing is strictly 1:1, so there is no set to size. A slot that boots empty is declared vacant explicitly, with the reason, on all three slot kinds that can: an `optional: true` participant slot, and a `zero_or_one` observer or producer slot. A topic slot receives from its bound producers and from no one else; a service or action call is answered by the one bound producer the caller selects, addressed directly. A consumer that needs several producers can declare several `one` slots (one role each) or one multi-cardinality slot. See [Why an explicit single target?](/advanced_guides/services/#why-an-explicit-single-target).
:::
...
- Use an **observer slot** when a third node only wants to watch one side of a pairing, like a recorder logging an arm's states. It taps the topics the observed role emits, follows that source instance rather than any pair, and never talks back.
