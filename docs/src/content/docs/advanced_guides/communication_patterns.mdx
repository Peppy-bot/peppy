...
| [Contract dependency](/advanced_guides/contract_conformance/) | `depends_on.contracts` | Any node whose `conforms_to` names the contract | One (pinned binding) or any number of conforming producers (`from_any: true`) |
...
- Use a **contract** when several nodes implement the same contract (a hardware driver and its sim twin, several camera vendors) or when any number of producers should feed one slot (telemetry fan-in, monitoring). The consumer names the contract; the launcher picks the producers. See [Pairing vs. contracts](/advanced_guides/pairing/#pairing-vs-contracts-from_any) for the boundary on the other side.
...
- `robot_arm` also conforms to a `joint_state_source` **contract**; dashboards consume its state **topic** through a `from_any` slot, so every arm on the floor feeds the same monitor without disturbing any pairing.
