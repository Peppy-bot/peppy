...
| [Contract dependency](/advanced_guides/contract_conformance/) | `depends_on.contracts` | Any node whose `conforms_to` names the contract | The one conforming instance the launcher binds to the slot |
...
:::note
Every slot binds exactly one producer, and every declared slot must be bound (launch rejects unbound slots). A topic slot receives from its bound producer and from no one else; a service or action call is answered by that one producer, addressed directly. A consumer that needs several producers declares several slots, one per producer. See [Why exactly one producer?](/advanced_guides/services/#why-exactly-one-producer).
:::
...
- `robot_arm` also conforms to a `joint_state_source` **contract**; a dashboard declares one contract slot per arm on the floor and consumes each arm's state **topic** through it, feeding one monitor without disturbing any pairing.
