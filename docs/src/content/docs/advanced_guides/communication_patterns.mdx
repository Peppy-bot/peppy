...
|---|---|---|---|
| Direct node dependency | `depends_on.nodes` | Instances of that exact producer node | The instances the launcher binds to the slot (one or several) |
| [Interface dependency](/advanced_guides/interface_conformance/) | `depends_on.interfaces` | Any node whose `conforms_to` names the contract | The conforming instances the launcher binds to the slot (one or several) |
...
:::note
A topic slot receives from every producer bound to it and from no one else; a slot left unbound stays silent. For services and actions, every call is answered by exactly one producer: the slot must be bound to a single producer, and the call addresses it directly. See [Why exactly one producer?](/advanced_guides/services/#why-exactly-one-producer).
:::
...
- Use an **interface** when several nodes implement the same contract (a hardware driver and its sim twin, several camera vendors) or when several producers should feed one slot (telemetry fan-in, monitoring, via the launcher's array binding). The consumer names the contract; the launcher picks the producers. See [Pairing vs. interfaces](/advanced_guides/pairing/#pairing-vs-interfaces) for the boundary on the other side.
...
- `robot_arm` also conforms to a `joint_state_source` **interface**; dashboards consume its state **topic** through an interface slot the launcher binds to every arm on the floor, feeding one monitor without disturbing any pairing.
