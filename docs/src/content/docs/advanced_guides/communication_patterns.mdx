...
|---|---|---|---|
| Direct node dependency | `depends_on.nodes` | Instances of that exact producer node | One (pinned binding) or a launcher-bound set of matching instances (`from_any: true`) |
| [Interface dependency](/advanced_guides/interface_conformance/) | `depends_on.interfaces` | Any node whose `conforms_to` names the contract | One (pinned binding) or a launcher-bound set of conforming producers (`from_any: true`) |
...
:::note
A `from_any` slot's producers are listed explicitly in the launcher binding; a slot left unbound is valid but silent (it receives nothing, and calls on it fail before any wire work). For topics, a bound slot receives from every producer in its bound set. For services and actions, every call is still answered by exactly one producer: a call on a slot bound to several producers runs a discover-then-pin probe restricted to the bound set and delivers the request to a single responder, so a state-changing call never executes on several producers at once. See [Why discover-then-pin?](/advanced_guides/services/#why-discover-then-pin).
:::
...
- `robot_arm` also conforms to a `joint_state_source` **interface**; dashboards consume its state **topic** through a `from_any` slot, so the launcher can bind every arm on the floor into the same monitor without disturbing any pairing.
