...
- `GoalDecision::Reject(...)` (`GoalDecision.reject(...)` in Python) rejects it: the client still receives the `GoalResponse`, but no context is created and the accept loop transparently moves on to the next goal. This is where you enforce per-resource concurrency limits.
    ...
    .handle_goal_next_request({
        ...
        move |request| -> Result<move_arm::GoalDecision> {
            ...
            if busy.lock().unwrap().insert(request.data.arm_id) {
                ...
                Ok(move_arm::GoalDecision::Reject(move_arm::GoalResponse::new(false)))
            }
        }
    })
    .await
...
def decide(request):
    ...
    if arm_id in busy:
        # This arm is already moving; reject so the caller can retry later.
        return move_arm.GoalDecision.reject(move_arm.GoalResponse(accepted=False))
    busy.add(arm_id)
    return move_arm.GoalDecision.accept(move_arm.GoalResponse(accepted=True))
...
`get_result` is deliver-once: the first successful call consumes the goal's buffered result, and the result is retained only for a short grace period after the worker finishes, so call it promptly (the drain-feedback-until-closed loop shown above does this for you). If the worker abandons the goal without ever completing it (an early return or a panic), the framework closes the feedback stream and `get_result` returns an error instead of blocking forever.
