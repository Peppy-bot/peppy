...
- **Pinned** (the default — `from_any` absent or `false`): the slot must be bound. The code generator splices the bound producer's `instance_id` into the generated `fire_goal` as the call's `target_instance_id`, and the goal, cancel, result, and feedback channels all address that one producer. The validator rejects a pinned-unbound slot before the stack starts.
