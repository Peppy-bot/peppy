...
Use `ActionHandle::expose` to set up the action, then loop accepting goals. `handle_goal_next_request` returns a per-goal `GoalContext` (or `None` when you reject the goal). Spawn a worker per context so goals run concurrently; each context owns that goal's feedback, cancel signal, and result, so nothing crosses between goals:
...
fn main() -> Result<()> {
    NodeBuilder::new().run(|_args: Parameters, node_runner| async move {
        ...
        tokio::spawn(async move {
            loop {
                // Accept or reject the next goal. The closure decides
                // concurrency: e.g. reject a goal for a busy `arm_id`.
                ...
                let ctx = match maybe_ctx {
                    Ok(Some(ctx)) => ctx,
                    Ok(None) => continue,   // goal rejected; wait for the next one
                    Err(_) => break,        // goal stream closed (shutting down)
                };
...
async def run_action(node_runner):
    ...
    # Accept or reject the next goal. The decider decides concurrency:
    # e.g. reject a goal for a busy arm_id.
    def decide(request):
        print(
    ...
    while True:
        ...
        if ctx is None:
            break  # goal rejected, or the goal stream closed (shutting down)
...
The decider returns a `GoalDecision`:
- `GoalDecision::Accept(...)` (`GoalDecision.accept(...)` in Python) accepts the goal, replies to the client, and yields a `GoalContext`.
- `GoalDecision::Reject(...)` (`GoalDecision.reject(...)` in Python) rejects it: the client still receives the `GoalResponse`, but no context is created. This is where you enforce per-resource concurrency limits.
...
Cancellation is **auto-acknowledged** by the framework: the client's `cancel()` returns `accepted = true` if a goal with that `goal_id` is in flight (the signal was delivered), or `false` otherwise. "accepted" means *delivered*, not *will stop*; a worker is free to ignore the signal and keep running. The worker decides the goal's fate: calling `complete_cancelled` (or `complete`) is what a subsequent `get_result` returns. Whichever completion runs first wins; the framework closes this goal's feedback stream on completion, so the client's `on_next_feedback_message` loop ends cleanly. A goal's cancel never affects other concurrent goals.
...
print(
    f"success={result.data.success} error={result.data.error_msg} "
...
Use `cancel_goal` on the handle to request cancellation of that specific goal:
...
error = cancel_response.data.error_message
print(
    f"cancel accepted={cancel_response.data.accepted} "
...
`accepted` reports whether the cancel signal was *delivered* to a live goal (the server's worker may still choose to ignore it), not whether the goal stopped. The cancel targets only this goal; other concurrent goals are unaffected. To learn the goal's final state, call `get_result`: if the worker reacted with `complete_cancelled`, `get_result` returns that cancelled result; if it ignored the cancel and finished normally, you get the normal result.
...
`GoalDecision::Reject`) when its target resource is already busy. A goal that is
not accepted yields no `GoalContext` and cannot be cancelled or completed.
