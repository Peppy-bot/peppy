...
The signature is the same for pinned and `from_any: true` slots: `fire_goal` takes no call-site targeting argument. The slot's launcher binding decides which producer instance the goal addresses; firing a goal on an unbound `from_any` slot fails before any wire work (see [Bindings and routing](#bindings-and-routing) below).
...
Routing for actions uses the same consumer-side model as topics and services. A binding `KEY: VALUE` is keyed by the slot's `link_id` and lists the producer instance(s) that fill the slot; the action server itself is binding-agnostic.

Each slot in `depends_on` is either:

- **Pinned** (the default, when `from_any` is absent or `false`): the slot must be bound to exactly one producer. The generated `fire_goal` resolves the bound producer's full `(core_node, instance_id)` wire address from the binding, and the goal, cancel, result, and feedback channels all address that one producer. The validator rejects a pinned-unbound slot before the stack starts.
- **`from_any: true`**: the binding lists the slot's explicit producer set — one instance, or several. Bound to exactly **one** producer, the generated `fire_goal` addresses that producer's `(core_node, instance_id)` directly, exactly like a pinned slot. Bound to **two or more**, `fire_goal` runs **restricted discovery**: one producer-pinned probe per bound producer, the first success wins, and the winner handles the whole goal cycle — goal, cancel, result, and feedback. The goal can never leave the bound set. Left **unbound** (key omitted, or bound to an empty array), the slot is valid but silent: `fire_goal` fails with an unbound-consumer-slot error before any wire work. There is no wildcard fallback.
...
```json5
{
  ...
  instances: [{
    ...
    bindings: {
      brain: "left-arm-1",              // pinned slot: exactly one producer
      arms: ["left-arm-1", "right-arm-1"],  // from_any slot: any number of producers
    },
  }],
}
```
...
```sh
peppy node run --bind brain@left-arm-1 --bind arms@left-arm-1 --bind arms@right-arm-1 .
```

### Per-call precedence

When several sibling slots of the same `(name, tag)` exist on a client, the slots never compete: each goal is routed by the binding of the slot it is fired on, and nothing else.

1. A goal fired on a **pinned** slot's module reaches that slot's one bound producer.
2. A goal fired on a **`from_any`** slot's module resolves within that slot's own bound set: directly when one producer is bound, via restricted discovery when several are.
3. A goal fired on an **unbound** `from_any` slot fails before any wire work.

Sibling bindings are independent: the same producer may be pinned by one slot *and* listed in a sibling `from_any` slot's bound set; goals on each slot are routed by that slot's own binding.

### Worked example: `openarm01_backbone`

A client that wires two specific depth cameras to dedicated wrist slots and reaches the remaining camera through a third, explicitly bound slot:
...
3. `extra_cam_<action>::fire_goal(...)` reaches `ceiling_cam` because the binding lists it. Bound to that single producer, the slot resolves to its full wire address and skips discovery entirely, exactly like a pinned slot.
4. Had `extra_cam` been bound to several producers (e.g. `extra_cam: ["ceiling_cam", "shelf_cam"]`), each `fire_goal` would run restricted discovery among them and the winner would drive the whole goal cycle. Left unbound, the slot is silent and `fire_goal` errors before any wire work.

### Discover-then-pin (multi-bound `from_any` slots only)

When `fire_goal` is called on a `from_any` slot bound to **two or more** producers, the framework runs a **discover-then-pin** sequence before sending the real goal:

1. A lightweight probe is sent to each bound producer — one producer-pinned probe per member of the bound set, never a wildcard broadcast. The probe is auto-handled by the framework before any user goal handler runs.
2. The first producer to respond is captured and the wire sender is pinned to its `(core_node, instance_id)`.
...
The non-winning producers see only the probe; their goal handler **never runs** — and a producer outside the bound set never even sees the probe. Without this step every bound producer would execute the goal concurrently, which for state-changing actions (motor commands, file writes, payment dispatches) is a real-world hazard.

Pinned slots (and `from_any` slots bound to a single producer) skip discovery entirely; no probe is sent. The binding carries the producer's full `(core_node, instance_id)` wire address, so the generated call addresses exactly one producer and the goal request has the caller's whole `goal_timeout` to itself. Probes for the multi-bound calls that do discover are answered by the transport adapter, so a producer busy executing a goal still answers discovery promptly.

The cost is one additional round-trip on multi-bound `fire_goal` calls (typically a few milliseconds). If the discovered producer dies between the probe and the real goal, `fire_goal` surfaces `ServiceUnreachable` and the caller can retry; the next attempt re-discovers a different producer from the bound set if one is available. Discovery is capped at the caller's `goal_timeout` if that is tighter than the framework's default probe timeout, so a fast-fail budget still surfaces unreachable producers quickly. An **unbound** `from_any` slot never reaches discovery at all: `fire_goal` fails before any wire work.

### Validator rules

The launcher validator runs these checks before the stack starts:

1. **Pinned-unbound is a hard error.** Every pinned slot in `depends_on` must have a binding whose `KEY` equals the slot's `link_id`, naming exactly one producer (an array on a pinned slot is rejected). A `from_any` slot has no such rule: leaving it unbound is valid and yields a silent slot whose goals fail before any wire work.
