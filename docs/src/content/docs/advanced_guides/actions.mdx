...
fn main() -> Result<()> {
    NodeBuilder::new().run(|_args: Parameters, node_runner| async move {
        ...
        let arms = brain_move_arm::bound_producers(&node_runner);
        let Some(arm) = arms.first() else {
            println!("no producer bound to the `brain` slot, no goal to fire");
            return Ok(());
        };
...
async def run(node_runner):
    ...
    arms = brain_move_arm.bound_producers(node_runner)
    if not arms:
        print("no producer bound to the `brain` slot, no goal to fire")
        return
    ...
    action_handle = await brain_move_arm.ActionHandle.fire_goal(
        node_runner,
        arms[0],                # which bound instance executes the goal
        request,
        5.0,                    # timeout (seconds)
        QoSProfile.SensorData,  # QoS for the feedback topic
    )
...
`fire_goal` requires the caller to select one member of the slot's `bound_producers()` set explicitly, for every [cardinality](/advanced_guides/topics#dependency-cardinality) including the default `one`. The target must belong to the slot's own set; anything else fails with a runtime error before the goal reaches the wire. Firing a goal at every bound producer of a multi-cardinality slot is a plain loop over `bound_producers()`; each returned handle drives its own feedback stream, cancel, and result, exactly as a single goal does.
