...
`fire_goal` takes no call-site targeting argument: the slot's launcher binding decides which producer instance the goal addresses. Every slot is bound to exactly one producer (a launch that leaves a slot unbound or binds it twice is rejected at validation), so a fired goal always has a resolved target.
