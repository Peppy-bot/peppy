...
The `feedback_topic` block is optional: actions that don't stream progress can omit it entirely. When you do declare `feedback_topic`, its `message_format` is what makes feedback usable: every feedback message must carry a non-empty payload, since empty payloads are reserved as the framework's per-goal end-of-stream signal (see [Receiving feedback](#receiving-feedback)). A `feedback_topic` without a `message_format` produces no working feedback publisher: in Rust the generated `publish_feedback` returns a `MessageFormatUnavailable` error at runtime, and in Python no `publish_feedback` method is generated at all.
