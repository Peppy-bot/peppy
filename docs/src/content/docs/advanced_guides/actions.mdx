...
Dependencies are declared in `manifest.depends_on` and referenced by `local_node_id` in the interface:
...
Like services and topics, `fire_goal` accepts an optional `target_variant`. When several variants of the same `name:tag` are running side by side (for example, two implementations of `arm_controller:0.1.0`), `Some("variant_name")` pins the goal to one variant; `None` lets any variant of the action server accept it. Pass `"default"` to target the baseline explicitly.

```rust
// Send the move_arm goal only to the production variant
let mut goal = brain_move_arm::fire_goal(
    &node_runner,
...
The `action_handle` carries the resolved variant, so subsequent `on_next_feedback_message`, `get_result`, and `cancel_goal` calls all stay pinned to the same server.
