...
`get_result` always resolves to a definitive answer: it never fails with a "no active goal" error and never blocks forever. A poll on a still-running goal **parks** until the goal finishes, so you can call it whenever you like; a forwarding/relaying node does not have to time its poll to the worker's lifecycle. The returned outcome (a Rust `ResultOutcome` enum, or `result.status` plus optional `result.data` in Python) tells you exactly what happened:
...
The result is **not** deliver-once: you can fetch it more than once within the retention window, which makes relaying it reliable. If the poll outlives the caller's own `timeout`, you get a normal timeout error like any other request.
...
The `CancelState` reports what the cancel found; there is no "no active goal" error:
