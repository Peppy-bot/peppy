...
The feedback stream is automatically closed by the server when it begins handling the result request, when it accepts a cancel request, or when its cancel handler errors. Each goal cycle has its own feedback stream — the close signal for one goal does not affect concurrent goals from other clients. Clients can rely on the loop-until-error pattern to drain feedback before requesting the result.
