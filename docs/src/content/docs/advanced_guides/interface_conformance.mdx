...
Interface conformance is the abstraction you reach for when several nodes implement the same contract. A `realsense_d405` driver, a `webcam` driver, and a `gazebo_camera_sim` all expose the same `video_stream` topic. Without conformance, every consumer would have to hard-code one of those producer names. With conformance, the consumer asks for the `depth_camera:v1` interface and the launcher decides which physical driver fills the slot.
...
A single node can conform to multiple interfaces (e.g. `[{depth_camera, v1}, {uvc_camera, v1}]`) — it then satisfies any consumer slot that asks for either.
...
The matching predicate at binding time is: *a producer satisfies an interface slot iff its `interfaces.conforms_to` includes the slot's `(name, tag)`*. The producer's own node name is irrelevant. A `webcam:v1` node that `conforms_to: [{ depth_camera, v1 }]` is just as valid for a `depth_camera:v1` slot as a `realsense_d405:v1` node that conforms.
...
├─ webcam_uvc:v1     (node)        conforms_to: [{depth_camera, v1}]
└─ gazebo_camera:v1  (node)        conforms_to: [{depth_camera, v1}]
...
```json5
{
  peppy_schema: "launcher_v1",
  deployments: [
    { source: { name: "realsense_d405:v1" }, instances: [
      { instance_id: "rs_left" },
      { instance_id: "rs_right" },
    ]},
    { source: { name: "webcam_uvc:v1" }, instances: [
      { instance_id: "fallback_webcam" },
    ]},
    { source: { name: "video_reconstruction:v1" }, instances: [{
      instance_id: "recon_1",
      bindings: {
        left_cam: "rs_left",
        right_cam: "rs_right",
        any_label: "fallback_webcam",
      },
    }],
    },
  ],
...
`left_cam` and `right_cam` are pinned interface bindings — the launcher checks `rs_left` and `rs_right` both conform to `depth_camera:v1` (they do, via the `realsense_d405:v1` producer node). `any_label` is a free-form binding key; the launcher attaches it to the `backup_cams` from_any slot because `fallback_webcam`'s node `(webcam_uvc:v1)` conforms to `depth_camera:v1`.

Swapping `realsense_d405:v1` for `gazebo_camera:v1` in the launcher requires no change to the consumer node — both producers conform to the same interface and the binding works unchanged.
