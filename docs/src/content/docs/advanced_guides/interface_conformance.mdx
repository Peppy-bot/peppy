...
A consumer references an interface through `manifest.depends_on.interfaces` rather than `depends_on.nodes`. Every interface dep carries a `link_id` (exactly like a node dep), but its `(name, tag)` names the interface contract instead of a concrete producer:
...
### Binding an interface slot

An interface slot is bound with `--bind link_id@producer_instance_id` (or a launcher `bindings:` entry) whose value points at an instance whose node conforms to the requested interface:
...
First, the consumer declares a single interface dep and consumes the topic through that slot's `link_id`:
...
`localizer` provides `pose_service` and *calls* `obstacle_service`; `mapper` provides `obstacle_service` and *calls* `pose_service`. Each node conforms to the interface it provides and consumes the service it calls through a `link_id`, exactly like a mutual-topic wiring, except both consumed links are **services**:
