...
A `from_any: true` interface slot accepts any number of conforming producers, all bound under the slot's `link_id` (an array in a launcher, or repeated `--bind link_id@...` flags): the slot receives from every producer in its bound set, and only those.

A `from_any` slot may also be left without a binding. It then materializes **unbound** and the node still boots — but the slot is silent: no subscription is created, nothing is received, and service or action calls on it fail before any wire work. Fan-in is always explicit: the launcher lists exactly the conforming producers that feed the slot, and the next launch can list a different set (or none) without touching the consumer. That property makes it the natural shape for optional many-to-one fan-in: telemetry, monitoring, any consumer whose input set is chosen deployment by deployment. (For an exclusive 1:1 bidirectional relationship between two specific instances, use a [pairing](/advanced_guides/pairing) instead.)
...
Every binding is keyed by a declared `link_id`: `left_cam` and `right_cam` each take exactly one conforming producer, and `extras` takes any number. The same per-message routing rules as node deps apply; see [Bindings and routing](/advanced_guides/topics/#bindings-and-routing) for the bound-set semantics.
...
Then the launcher binds all three producer instances under the slot's `link_id`, using the array form:
...
All three producer instances are listed under the `extras` key — the slot's declared `link_id` — and form that slot's bound set: the consumer receives messages from all three, and only these three, through the generated `extras_video_stream` module. From the command line the same set accumulates by repeating the flag: `--bind extras@rs_front --bind extras@rs_back_left --bind extras@rs_back_right`.

Inside the consumer, every consumed-topic API returns the **producer's full `(core_node, instance_id)` identity** alongside the message payload. The consumer never hard-codes the launcher's instance ids; it just uses the returned identity as a runtime key to keep per-producer state (latest frame, frame count, last-seen timestamp, etc.). New producer instances added to the slot's bound set in a future launcher show up automatically, with no consumer code change.
...
`left_cam` and `right_cam` are pinned interface bindings: the launcher checks `rs_left` and `rs_right` both conform to `depth_camera:v1` (they do, via the `realsense_d405:v1` producer node). `backup_cams` is the `from_any` slot's own `link_id`; the launcher accepts `zed_overhead` into its bound set because `zed_overhead`'s node (`zed_2i:v1`) conforms to `depth_camera:v1`. A second backup camera would simply join the array: `backup_cams: ["zed_overhead", "zed_shelf"]`.
