---
title: Variants & Flavours
description: Define alternative implementations of the same node
sidebar:
  order: 6
---

Variants let you define multiple implementations of the same node — each with its own execution — while sharing a single manifest and interface contract.
...
├── openarm01/             ← full node (manifest + interfaces + execution)
│   └── peppy.json5
├── unitree_g1/            ← full node (manifest + interfaces + execution)
│   └── peppy.json5
└── 1x/                    ← full node (manifest + interfaces + execution)
    └── peppy.json5
```
