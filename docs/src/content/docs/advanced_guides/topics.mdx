...
4. If the `extra_cam` binding line were removed, the slot would stay silent: `ceiling_cam` frames would reach no slot at all, since a producer named by no binding is ignored.

### Validator rules

The launcher validator runs these checks before the stack starts:

1. **Every `KEY` must name a declared slot.** A `--bind KEY@VALUE` (or launcher `bindings:` entry) whose `KEY` matches no `depends_on.{nodes,contracts}` `link_id` is rejected. There are no free-form keys. A declared slot with no binding is legal; it simply stays silent.
