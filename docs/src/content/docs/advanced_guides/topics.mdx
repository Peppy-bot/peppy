...
```json5
{
  ...
  interfaces: {
    topics: {
      expects: [
        {
          id: "camera_frame",        // Your chosen identifier for this expected topic
...
The module name is `<node>_<topic_name>` based on the `id` field — in this case `uvc_camera_video_stream`.
