...
The module name is `<local_node_id>_<topic_name>` based on the `local_node_id` field — in this case `uvc_camera_video_stream`.
...
fn main() -> Result<()> {
    NodeBuilder::new().run(|_args: Parameters, node_runner| async move {
        let (instance_id, variant, frame) = uvc_camera_video_stream::on_next_message_received(
            &node_runner,
        ...
        println!(
            "got {}x{} frame encoded as {} from {} (variant {})",
            frame.width, frame.height, frame.encoding, instance_id, variant
        );
...
- `target_variant` — optionally filter messages from a specific variant of the publishing node, or `None` for any.

It returns a tuple of:
- `instance_id` — the instance that published the message.
- `variant` — the variant of the publishing node (`"default"` for the baseline).
...
async fn receive_frames(node_runner: Arc<NodeRunner>) {
    loop {
        ...
        match result {
            Ok((instance_id, variant, frame)) => {
                tokio::spawn(async move {
                    println!(
                        "got {}x{} frame from {} (variant {})",
                        frame.width, frame.height, instance_id, variant
                    );
                });
            }
            Err(e) => {
...
When several variants of the same `name:tag` publish on a topic, you can pin the subscription to one variant. For example, two `depth_camera:0.1.0` instances may run side by side — one `realsense_d435`, one `realsense_d405` — and a downstream consumer might only care about one of them:

- **`None`** (default) — messages from any variant are received.
- **`Some("variant_name")`** — only messages from the named variant are received; pass `"default"` to target the baseline explicitly.

```rust
// Subscribe only to the d435 stream
let (instance_id, variant, frame) = depth_camera_video_stream::on_next_message_received(
    &node_runner,
    None,
    None,
    Some("realsense_d435"),
...
The returned `variant` field tells you which variant produced each message, which is useful when subscribing variant-agnostic and routing on the receiving side.
