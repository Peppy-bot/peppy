...
A node *exposes* a topic to publish messages, and other nodes *consume* that topic to receive them.
...
A node that receives messages declares what it consumes under `interfaces.consumes.topics`:
...
```json5
{
  ...
  interfaces: {
    consumes: {
      topics: [
        {
          id: "camera_frame",        // Your chosen identifier for this consumed topic
