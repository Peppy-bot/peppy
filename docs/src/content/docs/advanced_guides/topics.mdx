...
Every generated consumed topic, service, and action module also exposes `bound_producers()`, which returns the slot's runtime-resolved, immutable, ordered producer set (application declaration order, so `.first()` is deterministic). The set is fixed when the node starts and is not live discovery: a producer disconnecting does not shrink it, trigger discovery, or cause automatic rebinding.
...
The generated API is the same for every cardinality, including the default `one`: topics subscribe to the slot's complete bound set and yield the producing `ProducerRef` with every message, [services](/advanced_guides/services) and [actions](/advanced_guides/actions) require the caller to select one member of that set explicitly, and every consumed module exposes `bound_producers()`. For `one` the set contains exactly one member; for a `zero_or_more` slot bound to nothing, the subscription simply yields nothing until the node stops.
