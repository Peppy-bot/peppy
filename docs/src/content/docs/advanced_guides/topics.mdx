...
1. **Every `KEY` must name a declared slot.** A `--bind KEY@VALUE` (or launcher `bindings:` entry) whose `KEY` matches no `depends_on.{nodes,interfaces}` `link_id` is rejected. There are no free-form keys. A declared slot with no binding is legal; it simply stays silent.
2. **Every target must satisfy the slot.** For node slots, each target producer's `(node_name, node_tag)` must equal the slot's declared pair (`BindingTargetMismatch` otherwise). For interface slots, each target's `interfaces.conforms_to` must include the slot's `(name, tag)` (`BindingInterfaceNotConformed` otherwise); see [Interface conformance](/advanced_guides/interface_conformance) for the conformance contract.
