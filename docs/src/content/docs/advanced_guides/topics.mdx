...
| `zero_or_one`   | 0 or 1               | observer                  | error: link it, or write it `{ vacant: "<why>" }` |
| `one_or_more`   | 1 or more            | producer, observer        | error           |
| `zero_or_more`  | 0 or more            | producer, observer        | empty set       |

`zero_or_one` names an observed member set and is rejected on `depends_on.nodes` and `depends_on.contracts`: a producer slot binds its set once, when the node starts, and the accessor generated from the key is typed on that. It is the one observer cardinality a [vacancy](/guides/launch_files/#links-optional) is legal on, and its empty state is written rather than omitted, so a slot nobody mentioned stays an error.
