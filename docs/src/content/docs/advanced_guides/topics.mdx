...
A member of a `one_or_more` or `zero_or_more` slot is a `BoundMember`: its `producer` is the `ProducerRef` the slot's subscription tags messages with, and its `copy` is the [copy](/guides/launch_files/#copies) the producer's instance belongs to, `None` for an instance the launcher file deploys outside any copy. A node that holds members from several copies groups them by `copy`; one that addresses a member passes its `producer` to the slot's service and action calls. `peppy stack list` prints the copy beside each member (`alpha_arm_inst@cn-robot [copy alpha]`).
