...
import optionalReceiverRustConfig from '../guides/snippets/rust/optional_receiver/peppy.json5?raw';
import optionalReceiverRust from '../guides/snippets/rust/optional_receiver/src/main.rs?raw';

import optionalReceiverPythonConfig from '../guides/snippets/python/optional_receiver/peppy.json5?raw';
import optionalReceiverPython from '../guides/snippets/python/optional_receiver/src/optional_receiver/__main__.py?raw';
...
:::note
By default, the `node add` and `node sync` commands require the target node to already be in the node stack so the proper interfaces can be generated. The exception is a dependency whose slot admits an empty bound set (`cardinality: "zero_or_one"` or `"zero_or_more"`): `node add` resolves it through the repository cache automatically when it is absent from the stack.
...
- `None` (`Ok(None)` in Rust) once the node is shutting down and no queued message remains, or once the runtime is torn down.
...
Every generated consumed topic, service, and action module also exposes a bound-producer accessor for its slot, returning the producers bound to the slot when it is called, in plan order. The accessor's return type encodes exactly the guarantee [launch validation](#validator-rules) established for the slot's [cardinality](#dependency-cardinality):
...
pub fn bound_producers(node_runner: &crate::NodeRunner) -> NonEmptyProducers;

// Generated for every consumed module of a `cardinality: "zero_or_more"` slot:
// a plain, possibly empty list, so the empty branch is forced by the type
// and is never dead code.
pub fn bound_producers(node_runner: &crate::NodeRunner) -> Vec<ProducerRef>;

// Generated beside bound_producers() for every consumed module of a
// `one_or_more` slot: the same set, each member with the copy its instance
// belongs to.
pub fn bound_members(node_runner: &crate::NodeRunner) -> NonEmptyMembers;

// Generated beside bound_producers() for every consumed module of a
// `zero_or_more` slot.
pub fn bound_members(node_runner: &crate::NodeRunner) -> Vec<BoundMember>;
```

Python mirrors the split by name and annotation: a `one` slot generates `bound_producer(node_runner) -> peppylib.ProducerRef`, a `zero_or_one` slot `bound_producer(node_runner) -> Optional[peppylib.ProducerRef]`, and the multi cardinalities generate `bound_producers(node_runner) -> List[peppylib.ProducerRef]` (documented never-empty for `one_or_more`) and `bound_members(node_runner) -> List[peppylib.BoundMember]`.

A member of a `one_or_more` or `zero_or_more` slot is a `BoundMember`: its `producer` is the `ProducerRef` the slot's subscription tags messages with, and its `copy` is the [copy](/guides/launch_files/#copies) the producer's instance belongs to, `None` for an instance the launcher file deploys outside any copy. A `copy` carries the copy's `name` and the `instance_id` the copy's fragment wrote, which is the instance's name inside the copy. A node that holds members from several copies groups them by `copy.name` and names each one inside its copy by `copy.instance_id`; one that addresses a member passes its `producer` to the slot's service and action calls. `peppy stack list` prints the copy beside each member (`alpha_arm_inst@cn-robot [copy alpha]`).

An [observer slot](/advanced_guides/pairing/#observing-several-pairings-through-one-slot) types its own `source()` / `sources()` accessor the same four ways against its own cardinality.

Every module sharing the slot's `link_id` returns the same set in the same order. The set is the one the deployment binds: a producer disconnecting does not shrink it, trigger discovery, or cause automatic rebinding. On a `one_or_more` or `zero_or_more` slot the deployment changes it while the node runs when a [copy joins or leaves](/guides/launch_files/#copies), so the accessor returns the set as it is when called, and a `subscribe` follows it. A cardinality change is therefore visible at compile time in the consuming node: the accessor's type changes and every call site that relied on the old guarantee must be revisited. This is intended: a cardinality flip is a semantic change to the node.
...
fn main() -> Result<()> {
    NodeBuilder::new().run(|_args: Parameters, node_runner| async move {
        ...
        // `zero_or_one` written vacant, or `zero_or_more` bound to nothing:
        // the slot is empty; the subscription then yields nothing until the
        // node stops.
        let mut subscription = video_stream::subscribe(&node_runner).await?;
...
To follow a single camera instead, filter on the yielded producer (`bound_producers()` lists the members in plan order).
...
Every `depends_on.contracts`, `depends_on.nodes`, `depends_on.pairing_observers` and `depends_on.pairings` entry may declare a `cardinality`, constraining how many sources the application may bind to the slot: producers for the first two, [observed pairings](/advanced_guides/pairing/#observing-several-pairings-through-one-slot) for the third, [pairs](/advanced_guides/pairing/#pairing-with-several-peers-through-one-slot) for the fourth.
...
| Cardinality     | Valid unique targets | Slot kinds it is valid on       | Omitted binding | At join and remove |
| --------------- | -------------------- | ------------------------------- | --------------- | ------------------ |
| `one` / omitted | exactly 1            | producer, observer, participant | error           | unchanged |
| `zero_or_one`   | 0 or 1               | producer, observer, participant | error: link it, or write it `{ vacant: "<why>" }` | unchanged, except a participant slot a copy pairs into |
| `one_or_more`   | 1 or more            | producer, observer, participant | error           | grows and shrinks |
| `zero_or_more`  | 0 or more            | producer, observer, participant | empty set       | grows and shrinks |

`zero_or_one` is the one cardinality a [vacancy](/guides/launch_files/#links-optional) is legal on, on a producer slot, an observer slot and a participant slot alike: its empty state is written rather than omitted, so a slot nobody mentioned stays an error and "this rig deliberately has none" stays distinguishable from "whoever wrote this launcher forgot it".
...
Cardinality applies to a consumer's dependency slot; it does not permit undeclared or unbound launched node instances. On a producer-binding slot it constrains the bound set the deployment writes, not runtime availability: a producer disconnecting does not shrink the set, trigger discovery, or cause automatic rebinding. An observer slot's member set is delivered live by the daemon, so what the set says about each member moves while the node runs. On both kinds the members change only when the deployment does: a [copy](/guides/launch_files/#copies) joining adds its own instances to a running instance's `one_or_more` or `zero_or_more` slot, removing it takes them out, and the daemon delivers the whole new set, each producer member carrying the copy its instance belongs to, while a `one` or `zero_or_one` slot keeps what its launch bound. The planner checks every set it delivers against the slot's cardinality, and a removal that would empty a `one_or_more` slot is refused. A participant pairing slot is sized in pairs: its set is seeded at spawn, grows when a `stack join` pairs a copy into it and shrinks when a pair's other end stops. Its floor holds when the launch is planned, since the planner checks coverage then, and a read while the node runs can find fewer pairs, since a pair ends whenever either endpoint stops. `zero_or_one` is, on every slot kind, the node's own statement that a deployment may leave the slot empty, and the deployment still has to write down that it did.

The consuming API is uniform across cardinalities in everything except the [bound-producer accessor](#receiving-messages): topics subscribe to the slot's complete bound set and yield the producing `ProducerRef` with every message, and [services](/advanced_guides/services/) and [actions](/advanced_guides/actions/) require the caller to select one explicit, membership-checked member of that set, whatever its size. The accessor itself is cardinality-typed: `one` generates the singular, infallible `bound_producer()`, `zero_or_one` generates `bound_producer()` returning an `Option`, `one_or_more` generates `bound_producers()` returning a never-empty set whose `first()` needs no unwrap, and `zero_or_more` generates `bound_producers()` returning a possibly empty list, so the empty branch is forced by the type. An [observer slot](/advanced_guides/pairing/#observing-several-pairings-through-one-slot) types its accessor the same four ways: `one` generates the singular, infallible `source()`, `zero_or_one` generates `source()` returning an `Option`, `one_or_more` generates `sources()` returning a never-empty set, and `zero_or_more` generates `sources()` returning a possibly empty one. For an empty slot (a vacant `zero_or_one`, or a `zero_or_more` bound to nothing) the subscription simply yields nothing until the node stops.
...
<Tabs>
  <TabItem label="Python">
    <Code code={optionalReceiverPythonConfig} lang="json5" title="peppy.json5" />
  </TabItem>
  <TabItem label="Rust">
    <Code code={optionalReceiverRustConfig} lang="json5" title="peppy.json5" />
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <Code code={optionalReceiverPython} lang="python" title="src/optional_receiver/__main__.py" />
  </TabItem>
  <TabItem label="Rust">
    <Code code={optionalReceiverRust} lang="rust" title="src/main.rs" />
  </TabItem>
</Tabs>
