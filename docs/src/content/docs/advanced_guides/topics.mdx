...
A node that receives messages declares what it expects under `interfaces.topics.expects`.
...
The code generator creates a module for each expected topic under `peppygen::expected_topics`.
...
```rust
use peppygen::expected_topics::uvc_camera_video_stream;
use peppygen::{NodeBuilder, Parameters, Result};
