...
A container node includes a `container` block inside its `execution` section instead of the usual `add_cmd` and `start_cmd` fields. The two are mutually exclusive — a node is either a container node or a process node, never both.
