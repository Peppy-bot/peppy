...
A container node includes a `container` block inside its `execution` section instead of the usual `build_cmd` and `start_cmd` fields. The two are mutually exclusive — a node is either a container node or a process node, never both.
...
Compare this with a standard process node, which defines `build_cmd` and `start_cmd` instead:
...
Container nodes don't need `build_cmd` or `start_cmd` — the definition file takes care of both building and running the node.
