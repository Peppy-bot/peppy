...
The resulting `.sif` file is stored under `~/.peppy/built_nodes/<name>_<tag>/`, named after the fingerprint of the staged sources (see [Build caching](#build-caching)), and is ready to be started.
...
:::note
A bind path that holds a `:` or a `,`, on the host side or the container side, is refused: Apptainer's `src:dest:opts` spec reads both characters as separators and offers no way to escape them, so such a path cannot be expressed. A path produced by `${parameters:...}` substitution is checked the same way, once the parameter resolves.
:::
...
Every `peppy node build`, including the ones `peppy stack launch` and
`peppy stack build` run for each deployment, fingerprints the staged node
tree before building: the node sources, `peppy.json5`, the def file, the
generated `.peppy/` code and the
...
When the fingerprint already has an artifact on disk, the build reuses it
...
The cache is a plain directory tree keyed by content, so it survives daemon
restarts and reboots, and a disk prepared on one machine carries its built
nodes to every machine that boots from it with the same peppy version, OS
and architecture. A process node's archive also embeds what the host
toolchain produced, which the fingerprint does not cover, so those machines
need a compatible build environment as well. Each node identity keeps
exactly one artifact: a build whose fingerprint differs replaces the
previous one.

Pass `--rebuild` to `peppy node build`, `peppy stack launch` or
`peppy stack build` to build from the staged sources even when a matching
artifact exists; the result replaces
it. Use it when something outside the fingerprint changed:

- A floating base image tag (`From: peppybot/rust-cargo-base:latest`) that
  now resolves to a different image. Pin the image with `@sha256:<digest>`
  in the def file to make base image changes part of the fingerprint.
- For a process node, the host toolchain or the environment `build_cmd` runs
  with.
...
Rust container builds reuse a shared cache across builds, without any
...
- [sccache](https://github.com/mozilla/sccache) caches rustc invocations via
  `RUSTC_WRAPPER`, so unchanged crates are not recompiled. The executable
  ships inside `peppybot/rust-cargo-base`, so nothing needs to be installed
  on the host, on Linux or macOS; the wrapper is activated only for defs
  that bootstrap from that image.

:::caution
Caching requires a `/peppy-cache` directory in the base image: build-time
binds cannot create their destination. The peppy base images
(`peppybot/rust-cargo-base`, `peppybot/python-uv-base`) ship it, and the daemon
enables caching automatically when a def file bootstraps from one of them.
Any other base image must create the directory itself (`RUN mkdir
/peppy-cache` in its Dockerfile) and the def file must mention
`/peppy-cache` to opt in; such builds get registry caching, and sccache
additionally requires the image to provide the executable on its PATH.
:::
...
build itself already runs with. On macOS, builds run inside the Lima VM:
the cache directory is bind mounted through it and the environment rides
the guest command, so container builds cache the same way as on Linux.
:::
...
A node that renders or computes on the host's NVIDIA GPU passes `apptainer_run_extra_args: ["--nv"]`: Apptainer then binds the host's driver libraries (CUDA, EGL, GLX, NVML) into the container at start. The bundled Apptainer's library list covers the desktop driver and the Tegra driver of Jetson boards alike, so the same node configuration renders through the GPU on both.
...
On Linux, a build reads registry credentials from the same file Apptainer reads: `$APPTAINER_AUTH_FILE` when it is set, else `~/.apptainer/docker-config.json` (written by `apptainer registry login`) when it exists, else `~/.docker/config.json` (written by `docker login`). Log in to the registry that holds your base image with either tool before you build.

Peppy gives Apptainer a copy of that file without the `https://index.docker.io/v1/access-token` and `https://index.docker.io/v1/refresh-token` entries that Docker's browser login adds. These entries hold Docker's own session tokens, not registry passwords, and Apptainer would otherwise use one of them at random in place of your Docker Hub credential. Peppy writes the copy before each build, so a new `docker login` applies to the next build without a daemon restart. If Peppy cannot read or parse the file, the build fails and names the file.

On macOS, builds run inside the Lima guest, so they use the credential files in the guest user's home directory.
