...
### Search for a contract's or pairing's users

```sh
peppy repo search <name>:<tag>
```

Shows who uses a contract or pairing: the nodes that implement it (`manifest.implements`), consume it (`depends_on.contracts`), play a role in it (`depends_on.pairings`), or observe one of its roles (`depends_on.pairing_observers`). The first lines name the published document the identity resolves to, with its repository, path, and sha256; contracts and pairings are separate namespaces, so an identity published as both is reported as both. One section per way of using it follows, grouped by repository like `peppy repo list`, each group a table with one row per slot: `NODE` and `TAG`, `ROLE` (pairing sections only), `SLOT` (the `link_id`, with its cardinality or `optional`), `PIN`, and `PATH` (the manifest path). A table wider than the terminal gives up width from its widest columns first, never below a header's width, and wraps over-long cells onto continuation lines inside the outline; piped output is never wrapped.

```text
rgb_camera:v1
  contract published by https://github.com/Peppy-bot/contracts-hub.git (ref: main) at cameras/rgb_camera.json5 (sha256 a4e66999c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8)
...
The search reads this machine's caches, so it reflects the last `peppy repo refresh` and needs no daemon (`--core-node` has no effect). An identity nobody uses is an answer, not an error: the command says that no indexed node uses it and, when repositories are excluded, that one of them may have provided a user. `--json` prints the same report as one JSON document with `query`, `contract`, `pairing`, `implementers`, `consumers`, `participants`, and `observers`.
