...
typed clients, and it embeds the exact published bundle bytes as its served catalog. Exposures
that select actions publish the bundle only: the node generator does not support action-backed
MCP tasks yet, and a node must never advertise a surface it cannot serve.
