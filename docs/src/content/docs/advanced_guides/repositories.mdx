...
peppy repo index [PATH] --check --include-repositories    # also validate every listed exposure against its contracts
```
...
```sh
peppy repo index [PATH] --check --include-repositories
```
...
`--include-repositories` adds that check. After the structural check, every exposure the index
