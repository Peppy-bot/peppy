...
Several matching identities are listed with where each is stored, names the pattern matches outright first, then by name:
...
├──────────────┼───────────────────┼────────────────────────────────────────────────────────────┼────────────────────────────┼──────────┤
│ mcp exposure │ front_camera:v1   │ https://github.com/Peppy-bot/mcp-hub.git (ref: main)       │ cameras/front_camera.json5 │ 583f9ac9 │
...
The `SHA256` column shows the first eight characters of each document's fingerprint, as the `PIN` cells below do. A `@<sha256>` query takes the complete 64-character fingerprint; `--json` carries fingerprints whole.

A query that settles on one identity gets that identity's full report: either only one `name:tag` matches, or the name pattern spells exactly one of the matched names out in full, so `rgb_camera:v1` keeps its report even while the pattern also finds `sim_rgb_camera:v1` (those remaining matches are listed after the report). The report's first lines name each document of the identity with its kind, repository, path, and sha256; contracts and pairings are separate namespaces, so an identity published as both is reported as both, and one report covers them. For a contract or pairing, one section per way a node uses it follows: implementing it (`manifest.implements`), consuming it (`depends_on.contracts`), playing a role in it (`depends_on.pairings`), or observing one of its roles (`depends_on.pairing_observers`). Sections are grouped by repository like `peppy repo list`, each group a table with one row per slot: `NODE` and `TAG`, `ROLE` (pairing sections only), `SLOT` (the `link_id`, with its cardinality or `optional`), `PIN`, and `PATH` (the manifest path). A node, launcher, or MCP exposure has no usage sections; its published line is the whole answer. A table wider than the terminal gives up width from its widest columns first, never below a header's width, and wraps over-long cells onto continuation lines inside the outline; piped output is never wrapped.

`--full` prints that report for every matched identity instead of the match table, the way `apt search --full` prints every record whole: `peppy repo search 'rgb_camera:v[12]' --full` answers who still uses each version in one pass. An identity one repository publishes twice fails a full search the way it fails a settled one, since its report has no answer a launch would accept.
...
The search reads this machine's caches, so it reflects the last `peppy repo refresh` and needs no daemon (`--core-node` has no effect). A query nothing matches is an answer, not an error: the command says so and, when repositories are excluded, that one of them may have provided the item. `--json` prints the same answer as one JSON document with `query` (the parsed `raw`, `name`, `tag`, and `sha256` parts), `matches` (each with its `kind`, `name`, `tag`, `exact`, and `published` document), `detail` (the usage report with `implementers`, `consumers`, `participants`, and `observers`; `null` unless the query settles on one identity), `details` (one such report per matched identity under `--full`, in match order; empty otherwise), and `excluded`.
