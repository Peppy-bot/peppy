...
Repositories tell peppy where to look for nodes, launchers, interfaces, and pairings. When you run `peppy repo refresh`, peppy walks every configured repository and indexes:
...
- **`interfaces-hub`** (`https://github.com/Peppy-bot/interfaces-hub.git`, tracked on `main`): shared interface definitions that nodes reference by `name:tag` to declare conformance.
...
|------|--------|-------------|
| `fs` | `path` | A local directory. Peppy recursively walks it, indexing every `peppy.json5` (node) and every `.json5` file whose body declares `peppy_schema: "launcher/v1"` (launcher), `peppy_schema: "interface/v1"` (interface), or `peppy_schema: "pairing/v1"` (pairing). |
...
Re-scans all configured repositories and rebuilds the node, launcher, interface, and pairing indexes. `peppy repo update` is accepted as an alias. During refresh, peppy:
...
- Records every `peppy.json5` as a node, every `.json5` file whose body declares `peppy_schema: "launcher/v1"` as a launcher (file stem becomes the launcher name), every `.json5` file whose body declares `peppy_schema: "interface/v1"` as an interface (keyed by the `name:tag` in its manifest), and every `.json5` file whose body declares `peppy_schema: "pairing/v1"` as a pairing (also keyed by its manifest `name:tag`).
- Reports each discovered node, launcher, interface, pairing, and excluded repository in real time.
- Caches results in `~/.peppy/cache/nodes.json5`, `~/.peppy/cache/launchers.json5`, `~/.peppy/cache/interfaces.json5`, and `~/.peppy/cache/pairings.json5`.

On completion it prints a summary, for example `Repository refresh complete. 12 node(s), 3 launcher(s), 5 interface(s), 2 pairing(s) found.`
