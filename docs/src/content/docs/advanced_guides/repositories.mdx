...
- **Nodes**, identified by filename: every `peppy.json5` file whose body declares `peppy_schema: "node_v1"` is treated as a node config.
...
- **Interfaces**, identified by content: every `.json5` file whose body declares `peppy_schema: "interface_v1"` is treated as a standalone interface contract. See [Interface conformance](/advanced_guides/interface_conformance/) for how nodes claim conformance to or depend on these contracts.
...
- **`interfaces_hub`** (`https://github.com/Peppy-bot/interfaces_hub`, tracked on `main`): community-maintained interface contracts.
...
| `cache/interfaces.json5` | Index of interface contracts discovered across repositories |
...
- Records every `peppy.json5` as a node, every `.json5` file whose body declares `peppy_schema: "launcher_v1"` as a launcher (file stem becomes the launcher name), and every `.json5` file whose body declares `peppy_schema: "interface_v1"` as an interface contract.
...
Synchronized from node stack:
  - already_added_dep:0.1.0
Synchronized from repositories:
  - uvc_camera:0.1.0 (git)
  - lidar_sensor:1.0.0 (fs)
```

A dependency that is missing from **both** the node stack and every configured repository is a hard failure:

```text
dep `gps_module:2.0.0` not found in node stack or repository cache; run `peppy repo refresh`
```
