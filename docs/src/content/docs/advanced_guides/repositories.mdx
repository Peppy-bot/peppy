...
2. **Repository cache** (`~/.peppy/cache/packages.json5`) — consulted only when the stack does not have the dependency. Repo-resolved deps are listed under `Synchronized from repositories:`, each tagged with its source kind (`fs`, `git`, or `http`).
