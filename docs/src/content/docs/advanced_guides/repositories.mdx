...
- **`interfaces-hub`** (`https://github.com/Peppy-bot/interfaces-hub.git`, tracked on `main`): shared contract definitions that nodes reference by `name:tag` to declare conformance.
