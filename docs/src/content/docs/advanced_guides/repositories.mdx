...
- **MCP exposures**, identified by content: every `.json5` file whose body declares `peppy_schema: "mcp_exposure/v1"` is treated as an MCP exposure, regardless of its filename, keyed by the `name:tag` declared in its manifest. An exposure selects members of sha256-pinned contracts and gives them stable public names, MCP-facing prose, and operational policies; [`peppy repo exposure`](#publish-an-exposures-bundle-and-mcp-server-node) validates it and publishes its bundle. See [MCP exposure](/advanced_guides/mcp/).
...
- Records what each repository's committed [`peppy_repository.json5`](#the-repository-index) publishes: nodes and their tags, launchers (one per name, no tag), and contracts, pairings, and MCP exposures (keyed by their manifest `name:tag`). A repository without that file fails its read with instructions to run `peppy repo index`.
...
### Publish an exposure's bundle and MCP server node

```sh
peppy repo exposure PATH           # validate PATH, write its <stem>.bundle.json sibling, and generate its node
peppy repo exposure PATH --check   # verify the committed artifacts instead of writing them
```

An MCP exposure document (`peppy_schema: "mcp_exposure/v1"`) selects members of sha256-pinned
contracts and assigns them stable public names, MCP-facing descriptions, and operational policies;
[MCP exposure](/advanced_guides/mcp/) is the guide to writing one and deploying its server node.
`peppy repo exposure` resolves each pinned contract through the local repository caches (run
`peppy repo refresh` first), validates the exposure against exactly those bytes, and publishes two
artifacts next to the document: the derived bundle (the public catalog of resources, tools, and
tasks with their JSON Schemas, plus the identity and contract slots of the generated MCP server
node), and the generated node itself in a sibling directory named after it. The node is a complete
Rust crate with no hand-written code: its `peppy.json5` declares one `depends_on.contracts` slot
per logical target, its `main` composes the shared `peppy-mcp-runtime` crate from
`public-peppy-libs`, its bridges translate between the canonical public JSON and the peppygen
typed clients (topics become resources, services become tools, and actions become tools backed by
MCP tasks), and it embeds the exact published bundle bytes as its served catalog.

Validation refuses, naming every violation at once, when a selected member does not exist in its
contract with the expected kind, a public name is claimed twice, a message definition does not
translate under the canonical schema mapping, a representation policy names a missing or wrongly
typed member, a `restrict` bound does not fit its member, a size limit is smaller than a payload's
finite maximum, or a pin does not match the resolved document's bytes.

`--check` regenerates everything and refuses committed files that do not match byte for byte,
listing every drifted file. Run it in CI alongside `peppy repo index --check` so a hub cannot
merge artifacts that have drifted from their exposure document. Only generated files are compared;
files a hub commits alongside them (such as a `Cargo.lock`) are not judged. Neither form needs a
running daemon. The remedy for a drift is the same as for the index: re-run
`peppy repo exposure PATH` and commit the result.
