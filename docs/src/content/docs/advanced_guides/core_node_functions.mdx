...
The graph's `edges` list the dependency relationships, with each edge pointing from one node entry to another. The list covers both direct `depends_on.nodes` dependencies and those resolved through [contract conformance](/advanced_guides/contract_conformance): a conformance edge carries a `via_contract` of `name:tag` (the contract it was resolved through), while a direct edge has none.
