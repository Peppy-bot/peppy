...
Each node entry carries its `name`, `tag`, `config_path`, optional `artifact_path`, `stage` (`Added`, `Building`, `Ready`, or `Root`), `variant` (always populated, `"default"` for the baseline), and its `instances`.
