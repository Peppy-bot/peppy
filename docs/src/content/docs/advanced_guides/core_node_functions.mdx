...
<Tabs>
  ...
  <TabItem label="Rust">
    ...
    fn main() -> Result<()> {
        NodeBuilder::new().run(|_args: Parameters, node_runner| async move {
            let result = stack_list(&node_runner, true, Duration::from_secs(3)).await?;
            ...
            for node in &result.graph.nodes {
                ...
                for instance in &node.instances {
                    // `state` displays as "starting" or "running".
...
<Tabs>
  ...
  <TabItem label="Rust">
    ```rust title="src/main.rs"
    let result = stack_list(&node_runner, false, Duration::from_secs(3)).await?;
...
:::note
Both `info` and `stack_list` default to a 10-second timeout. Pass an explicit timeout when calling from a latency-sensitive path so a slow or unreachable core node does not stall your node.
:::
