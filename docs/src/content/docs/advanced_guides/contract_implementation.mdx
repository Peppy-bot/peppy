...
After `peppy repo refresh`, the contract is cached and addressable by `(name, tag)`. Files use whatever filename you like as long as it ends in `.json5`; Peppy identifies a `contract/v1` document by its `peppy_schema` field. `peppy repo show <name>:<tag>` lists the indexed nodes that implement or consume it (see [Show what a query matches](/advanced_guides/repositories/#show-what-a-query-matches)).
...
A contract-backed entry is `{link_id, name}`, plus an optional `refine` block described [below](#refining-the-contracts-generic-arrays). The shape and QoS come from the contract document, never from the manifest: an inline `message_format`, `qos_profile`, or service/action payload field on a contract-backed entry is rejected at parse time. The `name` is a strict selector, byte-equal to a member the contract declares.
...
Implements link_ids share one flat namespace with `depends_on.{nodes,contracts,pairings,pairing_observers}` link_ids; a collision is rejected at parse time. Direction matters:
...
- `topics.consumes` may additionally reference a `depends_on.pairings` slot, naming a topic the counterpart role emits, or a `depends_on.pairing_observers` slot, naming a topic the observed role emits.
...
#### Refining the contract's generic arrays

A contract keeps an array generic when its length is a property of the implementing device rather than of the interface. A `limb_motion` contract declares `joint_positions: { $type: "array", $items: "f64" }` with "one entry per joint of the arm", because a contract for arms cannot say how many joints every arm has. The node that drives a particular arm knows, and says so with a `refine` block on the entry:
  ...
  exposes: [
    {
      ...
      // This arm has seven joints: pin the contract's generic joint vectors.
      refine: {
...
The block mirrors the member's structure down to the array and carries `{ $length: N }` at the leaf: a topic entry refines `message_format`, a service entry `request_message_format` and/or `response_message_format`, an action entry `goal_service`, `feedback_topic`, and/or `result_service`. Nested objects are stepped into by naming their fields, and an array of objects by `$items`:

```json5
refine: {
  message_format: {
    header: { frame: { $length: 4 } },
    transforms: { $items: { translation: { $length: 3 } } },
  },
},
```

The generated code is the contract's shape with those lengths set and nothing else changed: `[f64; 7]` instead of `Vec<f64>` in Rust, a length-checked `list[float]` in Python, exactly what a native `$length: 7` produces. Consumers keep generating from the contract as written and stay generic. The two interoperate because a fixed-length and a variable-length array share one wire encoding; `$length` only pins the refining node's types and adds its length checks. A goal whose array does not have the pinned length is rejected by the generated action server before the node's own goal handler runs, like any other goal that does not decode (see [Handling goals](/advanced_guides/actions/#handling-goals)). Refinements are not visible to anyone else: the MCP exposure schema, `stack benchmark`, and launch-time binding checks all read the contract document.

What a `refine` block may say is checked when the contract resolves, at `node add` / `node sync`, with every problem of an entry reported at once:

- every pinned path must name an array the contract declares, and the array must be generic: an array the contract already pins is refused, whether or not the lengths agree, because the contract is the place where that length lives;
- the array's items must be scalars (the same rule as a native `$length`): `string`, `bytes`, `time`, and object items cannot be pinned;
- `$items` steps into an array of objects only, and nested field names step into an object only.

```
entry `move_arm_joints` (link_id `moves`) refines contract `limb_motion:v1` in ways the document
does not admit: `goal_service.request_message_format.arm_id`: `$length` and `$items` apply to
arrays, but the document declares a `u8`; `result_service.response_message_format.position`:
the document already fixes the length at 3
```

A coverage failure on the slot is reported first; refinements are checked once the slot lists the right members. The block's own structure (a leaf that is exactly `$length`, no empty block, no shape modifier such as `$type` inside it) is checked at parse time, and `refine` on a native entry is rejected there as well: a native entry owns its shape and sets `$length` in its own `message_format`.

Refinement works the same way on the consuming side: a `depends_on.contracts` entry in `topics.consumes`, `services.consumes`, or `actions.consumes` may carry the same `refine` block, for a consumer that knows the device it will be bound to. A `depends_on.nodes` entry may not: a node dependency resolves to the producer's native shape, which is taken as is. Pairing-backed entries refine their pairing document's topics the same way; see [Pairing](/advanced_guides/pairing/#refining-the-pairings-generic-arrays).
...
The two instances must also read the same clock. A binding whose producer and consumer resolve to different [clock domains](/advanced_guides/system_clock/#clock-domains) is refused before anything starts, because the timestamps it would carry mean nothing on the other side.
...
How many implementing producers a slot binds is its declared [cardinality](/advanced_guides/topics/#dependency-cardinality): exactly one by default, at most one for `zero_or_one`, an application-selected set for `one_or_more` / `zero_or_more`. A consumer that reads from several implementing producers can either declare one contract slot per producer (giving each a distinct role in code) or one multi-cardinality slot bound to the whole set behind a single API. Both keep contract slots the natural shape for many-to-one consumption whose membership the application controls: telemetry, monitoring, any consumer that reads data from a launch-chosen set of producers. Every declared `one` / `one_or_more` slot must be bound, and a `zero_or_one` slot is bound or declared vacant; a launch that leaves one unfulfilled is rejected. (For an exclusive 1:1 bidirectional relationship between two specific instances, use a [pairing](/advanced_guides/pairing/) instead.)
