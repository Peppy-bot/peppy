...
After `peppy repo refresh`, the contract is cached and addressable by `(name, tag)`. Files use whatever filename you like as long as it ends in `.json5`; Peppy identifies a `contract/v1` document by its `peppy_schema` field. `peppy repo search <name>:<tag>` lists the indexed nodes that implement or consume it (see [Search the repositories](/advanced_guides/repositories/#search-the-repositories)).
