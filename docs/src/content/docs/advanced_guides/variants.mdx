...
:::note
Just like a regular `peppy node add`, adding a variant only stages it in the `Added` stage — it does **not** run the variant's `build_cmd` automatically. Chain the build with `peppy node add --variant mock ./robot_brain -b` (or run `peppy node build robot_brain:0.1.0` afterwards).
:::
