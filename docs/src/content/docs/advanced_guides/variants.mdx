...
Each `name:tag@variant` is a distinct entity in the stack: multiple variants of the same `name:tag` can run side by side. Consumers of the parent `name:tag` subscribe variant-agnostic and receive from every running variant; the received-message `variant` field lets user code disambiguate by source. The interface-consistency check at stack push enforces that every variant of a `(name, tag)` exposes structurally identical interfaces, so the variant-agnostic subscription is wire-safe.
...
If a variant omits `interfaces` entirely, no validation is performed and the root's interfaces are used as-is. This is the right choice for variants that ship alongside their root — they already share the root's contract. A flavor node consumed as a variant from a different project should declare its own `interfaces`, since its root and the consuming root can evolve independently and the declared block is what lets peppy compare them at add time.
...
### Picking a transitive dependency's variant

Dependency lookups are variant-agnostic by design: the parent's manifest only declares `(name, tag)` and the launch graph fans out to every running variant of that pair at runtime. To pick a specific variant for a transitive dependency, **add the dependency at the desired variant first**, then add the parent:

```sh
peppy node add depth_camera:0.1.0 --variant realsense
peppy node add lidar:0.2.0 --variant containerized
peppy node add my_robot:1.0.0 --variant mock
```

The parent (`my_robot:1.0.0@mock`) wires up to whichever variants of `depth_camera:0.1.0` and `lidar:0.2.0` are already in the stack. The interface-consistency check at stack push guarantees every variant of a `(name, tag)` exposes structurally identical interfaces, so the variant-agnostic dependency edge is wire-safe.
