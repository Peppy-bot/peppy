...
A launcher deploys exposures with `source: { exposures: [...] }`, listing each as `"<name>:<tag>"`. The deployment takes one argument, `port`, the loopback port the server [prefers](#several-servers-on-one-machine), and one `links` entry per target of every exposure it lists. Each link connects a logical role to a running instance.
...
The instance binds `127.0.0.1:<port>` (default `8900`) when that port is free, and serves each exposure at its own path:
...
`port` is a preference, not a reservation. A server that finds its port held by another process takes a port from the operating system instead, warns in its run log naming both ports, and announces the port it holds, so the `MCP endpoints:` block of the launch or the join and the `Instance endpoints` table of `peppy stack list` always carry the URL to connect to. That is what lets several copies of one robot fragment, each deploying the same exposures with the same `port`, run on one machine: the first holds the port the launcher names, and each of the others is reported where it listens. Servers starting together race for the port, one winning it, so a launcher that needs to know which URL is whose names a different `port` for each copy, and `--set-arguments <instance>.port=<n>` fixes one joined copy's. Only a held port falls back: any other bind failure stops the process before it becomes healthy, its run log says `cannot bind 127.0.0.1:<port>`, and the launch fails naming that instance. Read the log with [`peppy node info`](/guides/node_stack/#inspecting-a-node-with-peppy-node-info) on the node's identity.
