...
description: Tune the daemon's messaging topology, router ownership, peer-mode buffer sizes, and node lifecycle grace periods through peppy_config.json5.
sidebar:
  order: 9
---
...
:::note
The daemon reads its settings (`core_node_name`, `mode`, `peer`, `lifecycle`, `federation`, `zenohd`) **once, at daemon startup**. Editing them has no effect on a running stack; restart the daemon (`peppy service serve`, or `systemctl restart` your service) to apply changes. The `resource_servers` block is read fresh by each CLI auth command, so an edit there takes effect on the next `peppy auth login` without a daemon restart.
:::
...
- **Missing settings.** If the file exists but omits settings (typically a file written by an older peppy, before a newer knob existed), the daemon appends each missing defaulted setting with its default value and comments. Your own values, comments, formatting, and any unrecognized keys are otherwise preserved exactly as you wrote them; when appending defaults, completion may add a structural separator comma after the prior final entry. Every defaulted setting a peppy release understands is guaranteed to be covered by this mechanism (enforced by tests), so the file is brought up to the running release's full set of defaults on the next daemon start, and the daemon logs each setting it added. Fields required only by a non-default mode are not invented: for example, selecting `zenohd.mode: "external"` requires you to supply its `endpoint`.
...
A defaulted setting you delete from the file therefore comes back with its default on the next daemon start. To change one, edit its value instead of removing it. Removing a mode-specific required field instead makes the file invalid and leaves it untouched.

## The default file

```json5
// Read once when the peppy daemon starts, so edits below take effect only after
...
## `zenohd`: use an operator-run router

Leave `zenohd.mode` as `"managed"` (the default) for peppy to start and manage its bundled `zenohd`:
...
In managed mode, the configured messaging port must be free when the daemon starts, and peppy stops or restarts the router as needed.

To use a router you start yourself, select external mode and give peppy the Zenoh endpoint it should dial:
...
Adoption depends on the responsive router, not on how it was started. Login and logout leave an adopted router unchanged and report that federation is operator-managed. `PEPPY_MESSAGING_PORT` configures the listener for peppy's managed router; external mode uses the port written in `zenohd.endpoint` instead.
...
| `zenohd.mode` | `"managed"` | `"managed"` or `"external"` | Selects whether peppy owns the router lifecycle or adopts an operator-owned router. |
| `zenohd.endpoint` | — | Required in external mode; `tcp/<dial-host>:<port>`; no wildcard host | Network endpoint of the operator-run router. Peppy adopts it without changing its federation or managing its lifecycle. |

A value outside its constraint, an unsupported `zenohd.endpoint`, an unknown top-level or `zenohd` mode, or a syntax error all stop the daemon at startup with an error pointing at the problem (an out-of-range value names the offending field; a parse error reports the bad value or its position), so a typo can never silently revert your stack to defaults.
