...
description: Tune the daemon's messaging topology, router ownership, subscriber buffer sizes, and node lifecycle grace periods through peppy_config.json5.
sidebar:
  order: 9
---
...
- **Missing settings.** If the file exists but omits settings (typically a file written by an older peppy, before a newer knob existed), the daemon appends each missing defaulted setting with its default value and comments. Your own values, comments, formatting, and any unrecognized keys are otherwise preserved exactly as you wrote them; when appending defaults, completion may add a structural separator comma after the prior final entry. Every defaulted setting a peppy release understands is guaranteed to be covered by this mechanism (enforced by tests), so the file is brought up to the running release's full set of defaults on the next daemon start, and the daemon logs each setting it added. Fields required only by a non-default choice are not invented: for example, selecting `zenoh.zenohd.ownership: "external"` requires you to supply its `endpoint`.
...
A defaulted setting you delete from the file therefore comes back with its default on the next daemon start. To change one, edit its value instead of removing it. Removing an ownership-specific required field instead makes the file invalid and leaves it untouched.
...
// after restart. resource_servers is refreshed by each CLI authentication command.
{
  ...
  // The zenoh messaging transport: the local nodes' topology, peer-topology
  ...
  zenoh: {
    ...
    zenohd: {
      // "managed": peppy starts, monitors, and stops its bundled zenohd.
...
## `zenoh.local_nodes_topology`: local messaging topology

`zenoh.local_nodes_topology` selects how the nodes **on this machine** exchange data with each other:
...
## `zenoh.peer`: subscriber buffer sizes

Each subscriber buffers in-flight messages in a bounded local channel. The `zenoh.peer` block sets that channel's capacity (number of messages, not bytes) per QoS tier:
...
The capacities apply under both topologies, but they matter most in the peer topology, which is why they live under the `peer` block: with nodes peering directly there is no router relay between a publisher and a subscriber, so this buffer is all that absorbs a burst.
...
## `zenoh.federation`: cloud-router timeout
...
## `zenoh.zenohd`: use an operator-run router

Leave `zenoh.zenohd.ownership` as `"managed"` (the default) for peppy to start and manage its bundled `zenohd`:
...
Under managed ownership, the configured messaging port must be free when the daemon starts, and peppy stops or restarts the router as needed.

To use a router you start yourself, select external ownership and give peppy the Zenoh endpoint it should dial:
...
At startup, peppy requires a responsive Zenoh router at the configured endpoint. Nothing listening is a startup error; a non-Zenoh service at the address is a distinct startup error. A responsive router is adopted for observation only. Peppy never kills it on daemon exit, restarts it after a failure, or changes its configuration to federate it to the cloud router. The operator owns the router's lifecycle and federation configuration. If it becomes unresponsive while the daemon is running, the watchdog logs a prominent warning but leaves it alone; sessions reconnect automatically after the operator restores it.

Adoption depends on the responsive router, not on how it was started. Login and logout leave an adopted router unchanged and report that federation is operator-managed. `PEPPY_MESSAGING_PORT` configures the listener for peppy's managed router; external ownership uses the port written in `zenoh.zenohd.endpoint` instead.
...
| `zenoh.local_nodes_topology` | `"peer"` | `"peer"` or `"router"` | Topology of this machine's nodes: direct peer links with gossip, or relay everything through the central router. Cross-machine traffic always goes through the routers. |
| `zenoh.peer.standard_buffer_size` | `128` | `> 0` | Subscriber channel capacity (messages) for standard-QoS topics; also sizes service request/reply channels. |
| `zenoh.peer.high_throughput_buffer_size` | `1024` | `> 0` | Subscriber channel capacity (messages) for high-throughput-QoS topics such as sensor-data streams. |
...
| `zenoh.federation.connect_timeout_secs` | `30` | `>= 1` | Seconds the daemon spends resolving your organization's cloud router (at startup and on each auth login/logout) before falling back to a standalone router. |
| `zenoh.zenohd.ownership` | `"managed"` | `"managed"` or `"external"` | Selects whether peppy owns the router lifecycle or adopts an operator-owned router. |
| `zenoh.zenohd.endpoint` | — | Required with external ownership; `tcp/<dial-host>:<port>`; no wildcard host | Network endpoint of the operator-run router. Peppy adopts it without changing its federation or managing its lifecycle. |

A value outside its constraint, an unsupported `zenoh.zenohd.endpoint`, an unknown value for `zenoh.local_nodes_topology` or `zenoh.zenohd.ownership`, an unknown key inside `zenoh.zenohd`, or a syntax error all stop the daemon at startup with an error pointing at the problem (an out-of-range value names the offending field; a parse error reports the bad value or its position), so a typo can never silently revert your stack to defaults. Unknown top-level keys remain accepted and preserved.
