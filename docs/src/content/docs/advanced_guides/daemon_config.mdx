...
// after restart. resource_servers is refreshed by each CLI platform command.
{
  ...
  zenoh: {
    ...
    managed: {
      ...
      // Managed zenoh-router federation with the platform backend. An
...
This block configures the managed router's single federation path: the outbound mTLS upstream to the platform router, established by authentication. See [Platform](/advanced_guides/platform/) for the login flow, the report command, and namespace semantics.
...
Federation is unavailable under `zenoh.external`; in that mode, the router operator owns its listeners, certificates, and links.
