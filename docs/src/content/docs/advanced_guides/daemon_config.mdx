...
:::note
The daemon reads its settings (`core_node_name`, `zenoh`, `lifecycle`) from the active source **once, at daemon startup**. Editing them has no effect on a running stack; restart the daemon (`peppy service serve`, or `systemctl restart` your service) to apply changes. The `resource_servers` block in the active source is read fresh by each CLI platform command, so an edit there takes effect on the next `peppy platform login` without a daemon restart.
...
- **`connect_timeout_secs`** (default `30`, minimum `1`): how long the daemon spends resolving the platform router. Resolution happens at startup and whenever `peppy platform login`/`logout` asks the daemon to reapply its desired link. Router apply and TLS verification have separate bounded budgets. A failed attempt does not block daemon startup; the daemon reports the failure and retries in the background.

There are no certificate paths or CA switches in `peppy_config.json5`.
Production identity is automatic: login generates a local ECDSA P-256 key,
enrolls a short-lived certificate bound to the running `core_node_name`, and
stores it under
...
The per-node mTLS rollout is an intentional breaking cutover. Current Peppy
uses credentials schema v3 and rejects v2 or unversioned credentials with an
instruction to log in again. Old release builds have no per-core-node client
identity and their former one-way-TLS federation attempt is rejected once the
production router requires per-core-node certificates; no daemon setting
restores compatibility.

Federation application is unavailable under `zenoh.external`; in that mode,
the router operator owns its listeners, certificates, trust roots, links, and
rotation procedure.
...
`peppy platform login` and `logout` leave external federation untouched and
print a note instead of poking the daemon. In a production build, login still
uses the live daemon's fixed name and manages Peppy's local enrolled identity,
but it does not install that identity into an external router or verify the
router's federation link. The operator must configure mutually authenticated
...
`PEPPY_HOME`. The commands do not restart the daemon; a sign-in change reaches
its sessions on the next manual daemon restart. `PEPPY_MESSAGING_PORT`
configures the listener for peppy's managed router, while external mode uses
the port written in `zenoh.external.endpoint`.
...
This block holds the platform-backend base URL the CLI platform commands talk to. The daemon ignores it; the `peppy platform login` / `whoami` / `logout` commands read it. See [Platform](/advanced_guides/platform/) for the full login flow.
...
| `zenoh.managed.federation.connect_timeout_secs` | `30` | `>= 1` | Seconds the daemon allows for resolving desired federation links after the local router is ready. |
