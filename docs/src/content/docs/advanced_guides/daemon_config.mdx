...
The peppy daemon reads one global configuration file, `~/.peppy/conf/peppy_config.json5`. It controls the messaging topology of the whole stack, the subscriber channel buffer sizes, and the grace periods that govern node lifecycle. The daemon applies it to its own core-node session and to every node it spawns. The same file also records the backend resource-server URL the `peppy login` / `whoami` / `logout` commands talk to; that block is read by the CLI, not the daemon.

:::note
The daemon reads its settings (`mode`, `peer`, `lifecycle`) **once, at daemon startup**. Editing them has no effect on a running stack; restart the daemon (`peppy serve`, or `systemctl restart` your service) to apply changes. The `resource_servers` block is read fresh by each CLI auth command, so an edit there takes effect on the next `peppy login` without a daemon restart.
:::
...
// sizes) takes effect only after you restart the daemon.
{
  ...
  // Backend resource-server URL the `peppy login` / `whoami` / `logout`
...
This block holds the platform-backend base URL the CLI auth commands talk to. The daemon ignores it; the `peppy login` / `whoami` / `logout` commands read it. See [Authentication](/advanced_guides/authentication/) for the full login flow.
