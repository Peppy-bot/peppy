...
- **Locked-memory budget.** The kernel counts every mapped shared-memory segment against `RLIMIT_MEMLOCK`. peppy sizes its per-session segment (target 32 MiB) to the host's limit automatically and warns when the limit clamps it — payloads larger than the clamped segment fall back to the network path. For full-size segments on a systemd-managed host, set `LimitMEMLOCK=` accordingly (e.g. `infinity`) in the service unit; processes with `CAP_IPC_LOCK` (including root) are unlimited.
- **Containers.** Co-located container nodes only use shared memory if they share the host's IPC namespace and an adequately sized `/dev/shm` (budget roughly one 32 MiB segment per co-located publishing session against Docker's default 64 MiB `/dev/shm`). Containers in a separate network namespace (the Lima VM on macOS) are forced off shared memory automatically, like they are forced onto the router path.
...
| `shm` | `true` | `true` or `false` | Zero-copy shared memory for large payloads between co-located nodes; `false` forces the network path. |
