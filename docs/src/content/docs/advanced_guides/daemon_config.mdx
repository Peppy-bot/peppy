...
- **Missing settings.** If the file exists but omits settings (typically a file written by an older peppy, before a newer knob existed), the daemon appends each missing setting with its default value and comments. Your own values, comments, formatting, and any unrecognized keys are otherwise preserved exactly as you wrote them; when appending defaults, completion may add a structural separator comma after the prior final entry. Every setting a peppy release understands is guaranteed to be covered by this mechanism (enforced by tests), so the file is brought up to the running release's full set of settings on the next daemon start, and the daemon logs each setting it added.
...
// sizes) takes effect only after you restart the daemon.
{
  ...
  zenohd: {
    // Absolute path or `~/...` to a zenohd binary you run yourself, or null to
...
Leave `zenohd.path` as `null` (the default) for peppy to start and manage its bundled `zenohd`. In this mode, the messaging port must be free when the daemon starts, and peppy stops or restarts the router as needed.

Set `path` to an absolute path or a path beginning with `~/` when you want peppy to use a zenoh router that you start yourself. The configured value selects external-router mode and appears in startup hints, but peppy never executes it or checks whether that file exists or is executable. Empty strings, relative paths, and `~user/...` paths are rejected when the config loads.

While `path` is set, peppy never starts any zenoh router. Start yours before the daemon and listen on the messaging port (`PEPPY_MESSAGING_PORT`, default `7448`) at `0.0.0.0` or `127.0.0.1`, for example:

```sh
/absolute/path/to/zenohd -l tcp/0.0.0.0:7448
```

At startup, peppy requires a responsive zenoh router on that port. Nothing listening is a startup error with a command hint based on `path`; a non-zenoh process holding the port is a distinct startup error. A responsive router is adopted for observation only. Peppy never kills it on daemon exit, restarts it after a failure, or changes its configuration to federate it to the cloud router. The operator owns the router's lifecycle and federation configuration. If it becomes unresponsive while the daemon is running, the watchdog logs a prominent warning but leaves it alone; sessions reconnect automatically after the operator restores it.

Adoption depends on the responsive router, not on how it was started. This means a bundled `zenohd` orphaned when a previous daemon was killed with `SIGKILL` is adopted if `path` is set on the next start. Login and logout also leave an adopted router unchanged and report that federation is operator-managed.
...
| `zenohd.path` | `null` (peppy-managed router) | Absolute path or `~/...`; `null` for managed mode | Selects an operator-run router that must already be serving the messaging port. Peppy adopts it without executing the path, changing its federation, or managing its lifecycle. |

A value outside its constraint, an unsupported `zenohd.path`, an unknown `mode`, or a syntax error all stop the daemon at startup with an error pointing at the problem (an out-of-range value names the offending field; a parse error reports the bad value or its position), so a typo can never silently revert your stack to defaults.
