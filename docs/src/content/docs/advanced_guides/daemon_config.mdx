...
The override applies to `peppy service serve` and to every platform command that reads daemon configuration: `peppy platform login`, `logout`, `whoami`, and `list`. `PEPPY_HOME` still controls credentials, repositories, and all other Peppy state. Edits to a file-form override take effect on the daemon's next restart, just like edits to the default file.
...
// after restart. resource_servers is refreshed by each CLI authentication command.
{
  ...
  zenoh: {
    ...
    managed: {
      ...
      // its federation, regardless of this setting (each node seeds gossip from
      // the local router, so direct peer links form only between co-located nodes).
      ...
      //              Each session binds a wildcard listener on an ephemeral port,
      //              reachable from the network.
      //   "router" - gossip off: all traffic relays through the central zenohd
      //              router, and a session binds no listener of its own.
...
`core_node_name` fixes the name of this daemon's [core node](/reference/concepts/#the-core-node). Leave it `null` (the default) and the daemon derives a stable, machine-specific name of the form `cn-{adjective}-{animal}`; set a string to pin an explicit one.
...
The value must be non-empty, at most **63 characters**, and use only the node-name character set (letters, digits, `_`, `-`); the literal name `self` is reserved and rejected. An invalid value stops the daemon at startup with an error instead of failing later when the core node boots. `peppy service serve --core-node-name <name>` overrides the config for one run (the flag wins over the file); if both are absent, the derived default is used.
...
The setting is local by construction, which is why it carries the `local_nodes_` prefix: each node seeds gossip discovery from the local router, so a peer session only ever discovers, and forms direct links with, sessions co-located on this machine. Traffic to and from nodes on other machines always relays through the local router and its [federation](#zenohmanagedfederation-cloud-router-timeout), whichever topology you pick.

In `"peer"` mode a session binds a wildcard listener (`0.0.0.0` on an ephemeral port) so Zenoh can advertise the host's routable interface addresses for those direct links; that listener is therefore reachable from the network, not confined to loopback. This widens an existing exposure class rather than opening a new one: the Zenoh transport is unauthenticated, and the managed router's own listener already binds `0.0.0.0`. Only `"router"` mode leaves a session with no listener at all, because with gossip off nothing needs to advertise a dialable address, so pick it for a deployment that must not accept inbound peer links.
...
`endpoint` is a **dial address**, not a listener bind address. It must use `tcp/<host>:<port>` with a hostname, an IPv4 address, or a bracketed IPv6 address, for example `tcp/router.internal:7448` or `tcp/[::1]:7448`. The port is required and must be from `1` through `65535`. Listener wildcards such as `0.0.0.0`, `[::]`, and `*` are rejected because Peppy cannot connect to them. Metadata/config suffixes and reserved delimiters (`?`, `#`, `;`, `=`) are rejected, and `tcp/` is the only supported transport.
...
This block holds the platform-backend base URL the CLI platform commands talk to. The daemon ignores it; the `peppy platform login` / `whoami` / `list` / `logout` commands read it. See [Platform](/advanced_guides/platform/) for the full login flow.
...
The URL is resolved in precedence order: `--api-url`, then `PEPPY_API_URL`, then `api` here. An empty block falls back to the build's default backend. In release builds, plain `http` is accepted only for local backends (loopback / `*.localhost`) and any other host must be `https`, validated when the command runs; debug builds accept remote plain `http` with a warning.

## Reference

| Setting | Default | Constraint | Effect |
|---|---|---|---|
| `core_node_name` | `null` (derived `cn-...`) | Non-empty, ≤ 63 chars, node-name charset, `self` reserved | Fixed name for this daemon's core node; must be unique across all daemons on the same router/federation or the daemon refuses to boot. Overridable per run with `--core-node-name`. |
