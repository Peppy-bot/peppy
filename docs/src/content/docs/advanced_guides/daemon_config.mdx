...
By default, the peppy daemon reads one global configuration file, `~/.peppy/conf/peppy_config.json5`. It sets the daemon's core-node name, selects either a peppy-managed or operator-run Zenoh router, controls the managed router's messaging behavior, and defines the grace periods that govern node lifecycle. The daemon applies it to its own core-node session and to every node it spawns. The same file also records the platform backend used by CLI authentication and by the daemon's enrollment, renewal, and federation controller. [`PEPPY_CONFIG`](#override-with-peppy_config) can supply a read-only alternative source.

:::note
The daemon reads its settings (`core_node_name`, `zenoh`, `lifecycle`, and
`resource_servers`) from the active source **once, at daemon startup**. Editing
them has no effect on a running stack; restart the daemon (`peppy service
serve`, or `systemctl restart` your service) to apply changes. CLI platform
commands read `resource_servers` afresh, but daemon enrollment, renewal, and
federation discovery continue using the backend captured at startup. Logout
revocation uses the API origin bound to the stored OAuth session and identity.

In production, that startup value is also the daemon's certificate identity.
`peppy platform login` and automatic renewal use the exact immutable
`core_node_name` recorded by the running daemon, not a freshly reread config
value. Changing the name therefore creates a new identity: restart first, then
log in again to enroll a certificate for the new name.
:::
...
// after restart. resource_servers is refreshed by each CLI platform command.
{
  ...
  // In production this exact startup value is the per-core-node mTLS identity;
  // changing it requires a daemon restart and a new platform login/enrollment.
  ...
  zenoh: {
    ...
    managed: {
      ...
      // direct links over loopback). While logged in to the platform, node
      // sessions always relay through the router regardless of this setting.
      ...
      // Managed zenoh-router federation with the platform backend. A release
      // daemon holds exactly one outbound mTLS upstream using its automatically
      // enrolled, short-lived per-core-node certificate; logged out, missing,
      // mismatched, or expired identity leaves it standalone (never one-way TLS).
      federation: {
        // Seconds the daemon spends resolving the platform router before giving
  ...
  // Backend resource-server URL used by CLI authentication and by the daemon's
  // identity controller. Baked in at compile time (the dev backend in debug
  // builds, prod in release); PEPPY_API_URL overrides it for either process,
  // while CLI --api-url affects only that CLI command.
  resource_servers: {
...
A release daemon captures the resolved name in its startup state, and that
exact value is the identity authorized by its platform client certificate.
Enrollment never guesses the name in a separate CLI process. Consequently,
production `peppy platform login` requires a live daemon state. Editing
`core_node_name` while the daemon is running does not rename it, and an existing
certificate for the old name is rejected for the new daemon. To rename a
federated daemon, restart it with the new value and run `peppy platform login`
again.
...
The value must be non-empty, at most **63 characters**, and contain only ASCII letters, digits, `_`, or `-`. An invalid value stops the daemon at startup with an error instead of failing later when the core node boots. `peppy service serve --core-node-name <name>` overrides the config for one run (the flag wins over the file); both absent falls back to the derived default.
...
## `zenoh.managed.federation`: platform federation

This block configures the managed router's single federation path: the outbound
mTLS upstream to the platform router, established by authentication. See
[Platform](/advanced_guides/platform/) for enrollment, storage, status, logout,
and namespace semantics.

- **`connect_timeout_secs`** (default `30`, minimum `1`): the backend HTTP and identity-resolution bound used for enrollment, renewal, federation discovery, and remote logout preparation. Startup and login use it while resolving the desired platform link; router apply and TLS verification have separate bounded budgets. A failed startup attempt does not block the daemon; it reports the failure and retries in the background.

There are no certificate paths or CA switches in `peppy_config.json5`.
Production identity is automatic: the daemon's identity controller generates a
local ECDSA P-256 key, enrolls a short-lived certificate bound to the running
`core_node_name`, and stores it under
`<PEPPY_HOME>/conf/platform-core-node/generations/<generation-id>/`. The daemon
renews at the certificate's server-provided `renew_after` time (initially around
12 hours into a roughly 24-hour lifetime, with jitter), rotating the private key
each time. New generation paths force the managed router to reload. Peppy keeps
the previous valid generation until a real mTLS probe succeeds, and rolls back
to it if the new generation cannot connect.

Renewal and platform config pulls use the daemon process's authentication
environment. For PAT-based operation, put `PEPPY_API_KEY` in the daemon's
systemd/container/service environment, not only in the interactive shell that
runs `peppy platform login`. The PAT itself remains environment-only; the
enrolled key and certificate are persisted. Remove the PAT from both
environments before running `peppy platform logout`, which otherwise refuses
before changing any state.

Release builds fail closed. They use the operating system WebPKI roots and DNS
hostname verification for the platform router, and require a complete, valid
certificate/key pair for the exact API origin, account, workspace, and running
core-node name. Missing, unreadable, malformed, mismatched, not-yet-valid, or
expired material leaves the router standalone with an actionable error. There
is no unauthenticated or one-way-TLS fallback. A failed renewal retries with
backoff while the old certificate remains valid; once it expires, Peppy
de-federates until it can obtain and verify a replacement.

Debug builds instead retain the embedded shared development CA and client
identity for zero-configuration local testing. This path is compile-time
debug-only and cannot be enabled in a release build. Production must never
trust the development CA.

:::caution[The namespace is not an ACL]
mTLS authenticates a server-issued core-node identity, but the router does not
yet enforce per-workspace Zenoh permissions. The workspace namespace is routing
context, not a tenant security boundary; use production federation only between
mutually trusted nodes until server-side ACLs are implemented.
:::

Peppy uses the strict clean-break credentials contract `"version": 1`.
Malformed, unversioned, or differently versioned credentials are rejected and
left untouched during normal operation; there is no migration path because this
format has not been deployed to production. Explicit `peppy platform logout
--offline` may reset an invalid document only after proving the daemon is
stopped and acquiring its owner lock. No daemon setting relaxes per-core-node
mutual TLS or enables unauthenticated or one-way-TLS federation.

Peppy-managed router application and verification are unavailable under
`zenoh.external`; in that mode, the router operator owns its listeners,
certificates, trust roots, links, and router-side rotation procedure.
...
At startup, peppy requires a responsive Zenoh router at the configured endpoint. Nothing listening is a startup error; a non-Zenoh service at the address is a distinct startup error. Peppy observes a responsive router's health but never kills it on daemon exit, restarts it after a failure, or changes its configuration to federate it to the platform router. The operator controls the router's lifecycle and federation. If it becomes unresponsive while the daemon is running, the watchdog logs a prominent warning but leaves it alone; sessions reconnect automatically after the operator restores it.

`peppy platform login` and normal `logout` still delegate to the daemon's
identity controller in external mode. The controller enrolls, renews, revokes,
and clears Peppy-owned local identity for the live daemon's fixed name, but its
router applicator reports `operator_managed`: it does not install that identity
into the external router, rewrite or restart the router, verify its federation
link, or claim de-federation. The operator must configure mutually authenticated
TLS, trust, renewal/reload, and fail-closed behavior for that router. Logout
does not remove identity material that the operator copied outside
`PEPPY_HOME`, so the operator must remove it separately. Peppy may restart its
own sessions when their namespace changes, but never the external router.
`PEPPY_MESSAGING_PORT` configures the listener for peppy's managed router,
while external mode uses the port written in `zenoh.external.endpoint`.
...
This block holds the platform-backend base URL used by both sides of the local
identity-control boundary: CLI authentication commands read it when they run,
and the daemon captures it at startup for enrollment, renewal, and federation
discovery. Logout revocation instead uses the origin stored with the current
OAuth session and certificate identity. See [Platform](/advanced_guides/platform/)
for the full login flow.

- **`api`** (default `https://api.peppy.bot` in release builds, `http://127.0.0.1:3000` in debug): the backend used by platform authentication and daemon identity lifecycle. There is no dev/prod selection at runtime; the file stores exactly the build's backend.

For a CLI command the precedence is `--api-url`, then `PEPPY_API_URL`, then
`api` here. The daemon has no per-command flag, so its precedence is
`PEPPY_API_URL`, then `api`; configure the variable in the daemon service
environment and restart it. Both sides of login must resolve the same origin.
An empty block falls back to the build's default backend. Plain `http` is
accepted only for local backends (loopback / `*.localhost`); any other host must
be `https`.
...
| `resource_servers.api` | `https://api.peppy.bot` (release), `http://127.0.0.1:3000` (debug) | `https`, or `http` for local hosts | Backend used by CLI authentication and captured by the daemon identity controller at startup. |
| `zenoh.managed.federation.connect_timeout_secs` | `30` | `>= 1` | Backend HTTP/identity-resolution bound for enrollment, renewal, federation discovery, and remote logout preparation. |
