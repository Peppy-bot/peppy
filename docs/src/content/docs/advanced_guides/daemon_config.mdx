...
// after restart. resource_servers is refreshed by each CLI authentication command.
{
  ...
  // a daemon whose name is already in use refuses to boot. At most 63 characters
  // from the node-name character set (start with a letter; letters, digits, `_`, `-`).
...
|---|---|---|---|
| `core_node_name` | `null` (derived `core-node-...`) | Non-empty, ≤ 63 chars, node-name charset | Fixed name for this daemon's core node; must be unique across all machines on the same router/federation (each machine runs one daemon) or the daemon refuses to boot. Overridable per run with `--core-node-name`. |
...
A value outside its constraint, an unsupported `zenoh.zenohd.endpoint`, an unknown value for `zenoh.local_nodes_topology` or `zenoh.zenohd.ownership`, an unknown key inside `zenoh`, or a syntax error all stop the operation that loaded the configuration with an error pointing at the problem (an out-of-range value names the offending field; a parse error reports the bad value or its position), so a typo can never silently revert your stack to defaults. Unknown top-level keys remain accepted and preserved.
