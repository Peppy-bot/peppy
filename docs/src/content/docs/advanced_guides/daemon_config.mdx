...
A value outside its constraint, an unsupported `zenohd.endpoint`, an unknown value for top-level `mode` or `zenohd.mode`, an unknown key inside `zenohd`, or a syntax error all stop the daemon at startup with an error pointing at the problem (an out-of-range value names the offending field; a parse error reports the bad value or its position), so a typo can never silently revert your stack to defaults. Unknown top-level keys remain accepted and preserved.
