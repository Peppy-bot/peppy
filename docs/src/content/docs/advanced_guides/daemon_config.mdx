...
// after restart. resource_servers is refreshed by each CLI authentication command.
{
  ...
  zenoh: {
    ...
    managed: {
      ...
      // Router federation settings. The timeout bounds resolution of your
      // organization's cloud router. The optional listener lets other
      // peppy-managed routers federate with this one over mutual TLS.
      federation: {
        ...
        // Optional inbound mTLS listener for user-managed server federation.
        // null disables inbound federation. Port 7449 avoids the managed
        // router's normal messaging port, 7448. Restart the daemon after changing
        // the listener or identity paths. See `peppy federation ca` for PKI setup.
        listen_endpoint: null,

        // Identity files for inbound and outbound user-managed federation.
        // null selects <PEPPY_HOME>/conf/federation/{cert.pem,key.pem,ca.pem}.
        cert_path: null,
        key_path: null,
        ca_path: null,
      },
...
## `zenoh.managed.federation`: router federation

This block configures two ways a managed router can federate: the platform backend link established by authentication, and user-managed mTLS links established with `peppy federation`. See [Server federation](/advanced_guides/federation/) for the complete PKI, listener, firewall, and command walkthrough.

- **`connect_timeout_secs`** (default `30`, minimum `1`): how long the daemon spends resolving the platform backend's cloud router. Resolution happens at startup and whenever an auth or federation command asks the daemon to reapply its desired links. Router apply and TLS verification have separate bounded budgets. A failed attempt does not block daemon startup; the daemon reports the failure and retries background reconciliation.
- **`listen_endpoint`** (default `null`): an optional inbound mTLS listener in the form `tls/<host>:<port>`, for example `"tls/0.0.0.0:7449"`. A wildcard host is valid for a listener. The port must be from `1` through `65535` and must differ from the managed router's messaging port, which defaults to `7448`. Plain TCP, missing ports, and metadata or endpoint fragments containing `?`, `#`, `;`, or `=` are rejected. `null` disables inbound user-managed federation.
- **`cert_path`** (default `null`): the PEM certificate chain presented by this router for inbound and outbound user-managed federation. `null` selects `<PEPPY_HOME>/conf/federation/cert.pem`.
- **`key_path`** (default `null`): the private key corresponding to `cert_path`. `null` selects `<PEPPY_HOME>/conf/federation/key.pem`.
- **`ca_path`** (default `null`): the PEM fleet CA used to verify peer certificates. `null` selects `<PEPPY_HOME>/conf/federation/ca.pem`.

The three identity paths are used only for user-managed mTLS links. An enabled listener requires all three files to exist when the daemon starts. The same identity also authenticates outbound links created by `peppy federation federate`. Paths containing `#`, `;`, or `=` are rejected because those characters would corrupt the Zenoh endpoint syntax.

Listener and identity-path changes take effect only after a daemon restart. Federation commands can add or remove outbound links without a manual restart because they update the saved registry and ask the running daemon to reapply it. User-managed federation is unavailable under `zenoh.external`; in that mode, the router operator owns its listeners, certificates, and links.
...
| `zenoh.managed.federation.listen_endpoint` | `null` | `tls/<host>:<port>`; port differs from the messaging port | Optional inbound mTLS listener for user-managed federation. |
| `zenoh.managed.federation.cert_path` | `null` | Path without `#`, `;`, or `=` | Router certificate chain; `null` selects `<PEPPY_HOME>/conf/federation/cert.pem`. |
| `zenoh.managed.federation.key_path` | `null` | Path without `#`, `;`, or `=` | Router private key; `null` selects `<PEPPY_HOME>/conf/federation/key.pem`. |
| `zenoh.managed.federation.ca_path` | `null` | Path without `#`, `;`, or `=` | Fleet CA certificate; `null` selects `<PEPPY_HOME>/conf/federation/ca.pem`. |
