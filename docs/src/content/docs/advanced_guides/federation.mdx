...
apart. An [`exposures` deployment](/advanced_guides/mcp/#placement) crosses the
same way: the pinned exposure documents and the contracts they reference, from
which the machine hosting the instance derives the same MCP server the
coordinator planned.

**A clock domain.** A [clock domain](/advanced_guides/system_clock/#clock-domains)
is supplied by one instance and read by any number, and those may sit on
different machines. The domain's ticks ride the `clock` topic of the machine
hosting its publisher, under a key carrying the domain's name and incarnation,
and a consumer addresses that whole identity wherever it runs. A daemon hosts
wall time and any number of simulated domains at once, so a launch spreads a
domain across its machines the way it spreads anything else. What placement
decides is the domain's identity, which names the machine its publisher runs
on.
...
`peppy node add`, `peppy node run`, `peppy node remove`, and `peppy node stop`
too, naming the launch and coordinator holding it, and `peppy stack list` shows a `Reserved for launch ...` line
...
- **A filesystem repository cannot back a deployment placed on another
  ...
  node from a git repository, which every machine can fetch. The same holds
  for an `exposures` deployment, whose exposure documents and contracts are
  what gets pinned.
...
A launcher whose `components` include a `zero_or_more` or `one_or_more` axis runs that axis as copies, added and removed while its other instances keep running. The coordinator snapshots the launcher at launch and composes every join against that snapshot:
...
A copy's name is its placement link and the prefix of every instance it creates. The whole copy runs on the machine `--place` names, or on the coordinator. Run join and remove on the coordinator, with `--core-node NAME` from another machine. A join reserves the machines it touches, the providers its instances link to, the machines supplying the clocks its instances read, and the machine of every instance whose set it grows. A join is refused when the machine of an instance whose set it grows is offline on the federation; a removal is not, and it names the sets it left unchanged there. A machine outside that set that is offline blocks neither a join nor a removal.

Launch replaces every participating slice. Join adds the copy's instances to its host's slice; remove stops the copy alone and refuses while another instance links to it. Reset cancels the operation in flight, waits for its cleanup, and clears the targeted slices; with `--federated` it clears every machine of the launch, which is also how the machines a cancelled join reached are cleared. See [copies](/guides/launch_files/#copies) for argument overrides and the single simulated robot the simulations host today.
