...
- **Same clock source on every participant.** Every machine of a launch serves
  the same simulated or wall time; a participant whose `peppy service serve
  --clock-source` disagrees with the launch is refused during preflight, before
  any stack is touched. See [One clock across a
  fleet](/advanced_guides/system_clock/#one-clock-across-a-fleet).
