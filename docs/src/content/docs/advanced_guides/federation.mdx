...
A launcher whose `components` include a `zero_or_more` axis runs that axis as copies, added and removed while its other instances keep running. The coordinator snapshots the launcher at launch and composes every join against that snapshot:
