...
At run time, `peppy node run` accepts those dot-paths as trailing arguments and rebuilds the nested structure: `peppy node run cam:v1 video.width=1280` fills `video.width` inside the `video` group. A string-typed leaf can also be referenced from a container's `mount_paths` with `${parameters:<path>}`; see [Using parameters in mount paths](/advanced_guides/containers/#using-parameters-in-mount-paths).
...
Arrays cannot have `$default`. The schema above parses, but interface generation rejects array parameters in both Rust and Python, so `peppy node sync` and `peppy node add` fail on a node that declares one; a usable parameter schema is built from primitives and groups.
