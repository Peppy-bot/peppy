...
Scalar-copy types (`bool`, the integer types, `f32`, `f64`) carry no presence bit, so `$optional` on them is rejected at interface generation; model an optional scalar as an optional `bytes`/`string` or restructure the message.
