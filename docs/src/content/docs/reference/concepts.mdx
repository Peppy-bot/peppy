...
When a node subscribes to an interface, it creates a dependency on the node that exposes that interface. The node stack:
...
The node stack can be exported in multiple formats:
