...
The stack-list API exposes the nodes and dependency edges as a serialized JSON graph for programmatic access. The `peppy stack list` command renders that graph as an operator-friendly table in each live core node's section.
