...
| `endpoints` | Optional map of the sockets the node binds during setup, keyed by label (lowercase letters, digits and underscores, starting with a letter). Each entry declares a `kind`, `page` for a URL opened in a browser or `mcp` for one an MCP client attaches to, and a non-empty `description`. The node announces the socket it bound under each label, and the daemon reports the URLs; a declared label that setup never announces fails the start |
...
- **Health**: Whether a running instance answers the core node's liveness probes, `healthy` or `unhealthy`; it turns `unhealthy` only after three consecutive missed probes (not applicable once the instance is terminal)
...
3. **Instance Lifecycle**: Manages the creation, health tracking, and removal of node instances. Running instances are continuously health-probed; a sustained run of missed probes flags an instance `unhealthy` but never removes it, so an instance leaves the stack only when it is explicitly stopped or removed. See [Instance health and lifecycle](/guides/node_stack/#instance-health-and-lifecycle) for details.
