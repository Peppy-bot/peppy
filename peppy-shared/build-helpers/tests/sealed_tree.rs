//! `peppy-shared` is a sealed tree: nothing inside it may reach outside it.
//!
...
//! tree's root. That is what lets `platform-backend` consume these crates from
//! the repository without building a line of `peppy` itself.
//!
...
/// The root of the sealed tree: the `peppy-shared` directory itself.
...
fn no_manifest_reaches_outside_the_tree() {
    ...
    assert!(
        violations.is_empty(),
        "peppy-shared is a sealed tree: its crates may depend on each other \
...
fn every_package_finds_its_workspace_root_inside_the_tree() {
    ...
    assert!(
        adrift.is_empty(),
        "peppy-shared is a sealed tree: a package here finds its workspace \
...
fn no_symlink_reaches_outside_the_tree() {
    ...
    assert!(
        violations.is_empty(),
        "peppy-shared is a sealed tree: a symlink leaving {root:?} smuggles \
...
const ROOT: &str = "/repo/peppy-shared";
const CRATE_DIR: &str = "/repo/peppy-shared/peppylib-rs";
...
fn outside_sources_workspaces_and_members_are_caught() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            escape("package.build", "../../crates/generator-internal/build.rs"),
            escape("lib.path", "../../crates/auth-internal/src/lib.rs"),
            escape("test.path", "../../crates/daemon-internal/tests/daemon.rs"),
...
fn a_root_a_member_and_a_package_naming_its_root_are_not_adrift() {
    let manifests = [
        manifest_in(ROOT, "[workspace]\nmembers = [\"peppylib-rs\"]\n"),
        manifest_in(CRATE_DIR, "[package]\nname = \"peppylib-rs\"\n"),
        manifest_in(
            "/repo/peppy-shared/json5-pretty",
            "[package]\nname = \"json5-pretty\"\n\n[workspace]\n",
        ),
        manifest_in(
            "/repo/peppy-shared/config-test-support",
...
fn a_package_with_no_root_inside_the_tree_is_adrift() {
    let manifests = [
        // A root above the tree is the one cargo must not be left to find, and
        // it excludes the tree, so a package that reached it would not build.
        manifest_in("/repo", "[workspace]\nexclude = [\"peppy-shared\"]\n"),
        // With no workspace root inside the tree, every package here is adrift.
        manifest_in(CRATE_DIR, "[package]\nname = \"peppylib-rs\"\n"),
        manifest_in(
            "/repo/peppy-shared/json5-pretty",
            "[package]\nname = \"json5-pretty\"\n",
    ...
    assert_eq!(
        ...
        vec![
            ...
            PathBuf::from("/repo/peppy-shared/json5-pretty"),
        ]
    );
}
...
fn the_walk_reports_symlinks_and_skips_tool_directories_even_when_linked() {
    ...
    let root = scratch.path().join("peppy-shared");
    let krate = root.join("json5-pretty");
    ...
    symlink(scratch.path(), root.join("target")).expect("target link");
...
fn a_path_that_leaves_and_returns_is_judged_by_where_it_lands() {
    let manifest = r#"
        [dependencies]
        config = { path = "../../peppy-shared/peppy-config-model" }
        lookalike = { path = "../../peppy-shared-fork/peppy-config-model" }
    "#;
    assert_eq!(
        escapes(manifest),
        vec![escape(
            "dependencies.lookalike.path",
            "../../peppy-shared-fork/peppy-config-model"
        )]
    );
}
