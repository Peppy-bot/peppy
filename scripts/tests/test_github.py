...
class TestGithubApi:
    ...
    def test_empty_body_non_204_is_error(
        self,
        github_client: httpx.Client,
        mock_api: respx.MockRouter,
    ) -> None:
...
class TestGithubRepoSlug:
    ...
    def test_from_ssh_remote(self, tmp_repo: Path) -> None:
    ...
    def test_from_https_remote(self, tmp_repo: Path) -> None:
        import subprocess
    ...
    def test_empty_env_falls_back_to_remote(self, tmp_repo: Path) -> None:
...
class TestParseReleaseResponse:
    def test_valid(self, fake_release_response: dict[str, Any]) -> None:
...
class TestDeleteAssetIfExists:
    ...
    def test_empty_asset_list(
        ...
        slug = RepoSlug(owner="owner", repo="repo")
        delete_asset_if_exists(github_client, 1, "any.tgz", slug)


class TestReplaceAndUploadAsset:
    def test_replaces_existing(
        self,
        github_client: httpx.Client,
        mock_api: respx.MockRouter,
        tmp_path: Path,
