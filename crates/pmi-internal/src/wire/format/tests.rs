//! Snapshot tests for the shared wire format functions. These pin the exact
...
fn topic_subscribe_targeted_native_iface() {
    ...
    assert_eq!(
        WireFormat::topic_subscribe(&receiver),
...
fn parse_topic_keyexpr_extracts_caller_addressing() {
    ...
    let parsed = WireFormat::parse_topic_keyexpr(key).expect("should parse");
...
fn parse_topic_keyexpr_roundtrips_through_topic_publish() {
    ...
    let key = WireFormat::topic_publish(&sender);
...
fn parse_topic_keyexpr_missing_core_node_errors() {
    let err = WireFormat::parse_topic_keyexpr("only_one_segment").unwrap_err();
    assert!(matches!(
        err,
        WireParseError::MissingSegment("caller_core_node")
    ));
}
...
fn parse_topic_keyexpr_missing_instance_id_errors() {
    let err = WireFormat::parse_topic_keyexpr("a/b/c").unwrap_err();
    assert!(matches!(
        err,
        WireParseError::MissingSegment("caller_instance_id")
    ));
}
...
fn service_request_publish_specific_target() {
    ...
    assert_eq!(
        WireFormat::service_request_publish(&sender, "abc123"),
...
fn service_request_publish_action_goal() {
    ...
    assert_eq!(
        WireFormat::service_request_publish(&sender, "rid_42"),
...
fn service_response_subscribe_native_iface() {
    let sender = sample_service_sender(ServiceKind::Service);
    assert_eq!(
        WireFormat::service_response_subscribe(&sender, "abc123"),
...
fn service_response_subscribe_action_result() {
    ...
    assert_eq!(
        WireFormat::service_response_subscribe(&sender, "rid_42"),
...
fn parse_received_request_round_trips_specific() {
    ...
    let parsed = WireFormat::parse_received_request(&receiver, request).expect("should parse");
...
fn parse_received_request_rejects_service_root_mismatch() {
    ...
    let err = WireFormat::parse_received_request(&receiver, request).unwrap_err();
...
fn action_feedback_publish_native_iface() {
    ...
    assert_eq!(
        WireFormat::action_feedback_publish(&recv, "goal_xyz"),
...
fn action_feedback_subscribe_targeted() {
    ...
    assert_eq!(
        WireFormat::action_feedback_subscribe(&sender, "goal_xyz"),
