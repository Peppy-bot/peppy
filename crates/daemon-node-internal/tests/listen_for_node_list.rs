...
use common::{
    CALLER_INSTANCE_ID, send_node_add_and_wait, start_daemon_node_with_mock_messenger,
    write_peppy_json5,
};
...
async fn listen_for_node_list_returns_succeeds() {
    ...
    let has_root = nodes.iter().any(|node| {
        node.get("name").and_then(|v| v.as_str()) == Some(&started_daemon.daemon_node_name)
            && node.get("tag").and_then(|v| v.as_str()) == Some("daemon-node")
    });
    assert!(
        has_root,
...
async fn listen_for_node_list_returns_dot_graph() {
    ...
    assert!(
        dot_graph.contains(&format!("{}:daemon-node", started_daemon.daemon_node_name)),
