...
use daemon_node::encoding::NodeStartFeedback;
...
async fn listen_for_node_start_success() {
    ...
    let started_daemon = start_daemon_node_with_real_messenger().await;
    ...
    // Add the node to the daemon node's node stack
    ...
    let (messaging_host, messaging_port) = started_daemon
        .caller_handle
        .messaging_endpoint()
        .await
    ...
    let found_instance = started_daemon.node_stack.find_by_instance_id(&instance_id);
    assert!(
...
async fn listen_for_node_start_timeout() {
    ...
    let started = start_daemon_node_with_health_timeout(Duration::from_secs(2)).await;
...
async fn listen_for_node_start_abandoned_action_does_not_block_next_goal() {
    ...
    use daemon_node::encoding::NodeStartGoal;
    ...
    let first_action_handle = ActionMessenger::send_goal(
        ...
        &started.daemon_node_name,
        daemon_node::names::NODE_START_ACTION,
    ...
    let first_goal_response =
        daemon_node::encoding::NodeStartGoalResponse::decode(&first_goal_response_payload)
