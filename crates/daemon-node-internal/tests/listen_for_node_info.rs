...
use common::{
    AbortOnDrop, CALLER_INSTANCE_ID, start_daemon_node_with_mock_messenger, write_peppy_json5,
};
...
async fn listen_for_node_info_has_instance_ids() {
    ...
    let node_handle = MessengerHandle::from_shared(Arc::clone(&started_daemon.shared_messenger));
