...
async fn send_node_start_and_wait_internal(
    ...
    let (caller_daemon_node, caller_instance_id) = if feedback_tx.is_some() {
        ("*", "*")
    } else {
        (daemon_node_name, CALLER_INSTANCE_ID)
    };
...
async fn start_daemon_node_with_messenger(
