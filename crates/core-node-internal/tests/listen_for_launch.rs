...
use core_node::encoding::{LaunchFeedback, LaunchGoal, LaunchGoalResponse, LaunchResult};
use core_node::names;
...
async fn listen_for_launch_configuration_succeed() {
    ...
    assert_eq!(
        uvc.read().expect("entity poisoned").instances().len(),
        2,
    ...
    assert_eq!(
        brain.read().expect("entity poisoned").instances().len(),
        1,
