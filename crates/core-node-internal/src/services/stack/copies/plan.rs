...
/// New instances, their direct providers, and the instances supplying the
/// clocks they read all take part in a join: a copy cannot bind to a domain
/// whose publisher is not there to supply it.
pub(super) fn join_dependencies(
    planned: &[PlannedDeployment],
    new_ids: &HashSet<String>,
    clock_publishers: &[Name],
) -> HashSet<String> {
    ...
    new_ids
        ...
        .cloned()
        .chain(providers)
        .chain(clock_publishers.iter().map(Name::to_string))
        .collect()
}
...
mod tests {
    ...
    /// as a producer, and with the instances supplying the clocks it reads. An
    /// instance already running that the copy names nowhere stays out of it.
    #[test]
    fn a_join_depends_on_its_new_instances_their_producers_and_its_clock_publishers() {
        ...
        assert_eq!(
            join_dependencies(&planned, &new_ids, std::slice::from_ref(&sim)),
            HashSet::from([
        ...
        assert_eq!(
            join_dependencies(&planned, &new_ids, &[]),
            HashSet::from(["alpha_arm_inst".to_owned(), "shared_inst".to_owned()]),
            "a join onto wall time depends on no clock publisher"
        );
        assert!(join_dependencies(&planned, &HashSet::new(), &[]).is_empty());
    }
}
