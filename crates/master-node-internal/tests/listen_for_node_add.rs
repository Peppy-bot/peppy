...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn listen_for_node_add_release_fingerprint_mismatch() {
    ...
    // Create a valid config fingerprint but a mismatched release fingerprint
    config::fingerprint::create_wrong_release_fingerprint(
        &config_path,
        Path::new(PEPPYGEN_OUTPUT_PATH),
    );
    ...
    assert!(
        !add_result.success,
        "node_add should fail when release fingerprint mismatches"
    );
    assert!(
        add_result
            .error_message
    ...
    assert!(
        add_result
            .error_message
            .as_ref()
            .map(|msg| msg.contains("Release fingerprint mismatch"))
            .unwrap_or(false),
        "error message should mention release fingerprint mismatch, got: {:?}",
        add_result.error_message
    );
    ...
    assert!(
        !node_stack.contains(TARGET_NODE_NAME, TARGET_NODE_TAG),
        "node should not be added when release fingerprint mismatches"
    );
    assert_eq!(node_stack.len(), 1, "only root should exist");
