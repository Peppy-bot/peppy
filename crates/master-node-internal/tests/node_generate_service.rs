...
async fn test_node_generate_success() {
    let (client, _server) = setup_test_master_node().await;

    let temp_dir = Builder::new()
        .prefix("node_generate")
        .tempdir()
    ...
    let request = NodeGenerateRequest::new(&node_root_dir).with_build_system(BuildSystem::Rust);

    let node_generate_response = request
        .poll(
    ...
    assert!(node_generate_response.success);
    assert!(
        node_generate_response.error_message.is_empty(),
        "expected empty error message, got: {}",
        node_generate_response.error_message
    );
