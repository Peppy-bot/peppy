...
mod peppylib_build {
    ...
    ///
    /// Everything registered here is a source file this script only reads. The
    /// `.so` cache this script writes is deliberately absent: cargo anchors a
    /// `rerun-if-changed` check at the instant the build-script job starts, so a
    /// file the script writes while running is forever newer than that anchor and
    ...
    /// recompiles this crate; the `const` block then holds the marker against the
    /// provenance recorded here and fails the build with an explicit message
    /// instead of shipping bindings that no longer match the sources compiled
    /// around them.
    fn generate_embedded_peppylib_so(
        so_dir: &Path,
        state: &BTreeMap<String, (String, String)>,
        excluded_so: &[String],
    ) {
        ...
        for so in &so_files {
            ...
            let (hash, profile) = state.get(platform).unwrap_or_else(|| {
                panic!("embedding {name} without a recorded build state in {so_dir:?}")
            });
            provenance.push_str(&format!(
                "    {:?},\n",
                format!("{platform}\t{hash}\t{profile}\n")
            ));
        }

        let marker = so_dir.join(BUILD_STATE_MARKER);
        let content = format!(
            "// @generated by generator-internal/build.rs. Platform-suffixed peppylib\n\
             ...
             \n\
             // The build state of the shared `.so` cache, read at compile time so\n\
             // cargo tracks it as an input of this crate.\n\
             const PEPPYLIB_SO_BUILD_STATE: &str = include_str!({marker:?});\n\
             \n\
             // The `<platform>\\t<source hash>\\t<profile>` line each embedded\n\
             // binding was built from, as recorded when it was built.\n\
             const PEPPYLIB_EMBEDDED_PROVENANCE: &[&str] = &[\n{provenance}];\n\
             \n\
             const _: () = {{\n\
             \x20   let mut index = 0;\n\
             \x20   while index < PEPPYLIB_EMBEDDED_PROVENANCE.len() {{\n\
             \x20       assert!(\n\
             \x20           build_state_records(\n\
             \x20               PEPPYLIB_SO_BUILD_STATE,\n\
             \x20               PEPPYLIB_EMBEDDED_PROVENANCE[index],\n\
             \x20           ),\n\
             \x20           \"the shared peppylib .so cache no longer records the sources an \\\n\
             \x20            embedded binding was built from: another checkout of this \\\n\
             \x20            repository rebuilt the cache from its own sources. Rebuild the \\\n\
             \x20            bindings for this checkout with `PEPPYLIB_REBUILD=1 cargo build`.\"\n\
             \x20       );\n\
             \x20       index += 1;\n\
             \x20   }}\n\
             }};\n"
    ...
    ///
    /// Returns the verified build state, which records the provenance of every
    /// binding the caller then embeds.
    fn verify_prebuilt_so(so_dir: &Path, current_hash: &str) -> BTreeMap<String, (String, String)> {
    ...
    pub fn run() {
        ...
        if let Some(prebuilt) = std::env::var_os(PREBUILT_SO_DIR_ENV) {
            ...
            let state = verify_prebuilt_so(&so_dir, &current_hash);
            generate_embedded_peppylib_so(&so_dir, &state, &[]);
        ...
        generate_embedded_peppylib_so(&so_dir, &state, &stale);
