...
fn rendered_schema(name: &str, format_json5: &str) -> String {
    let format = parse_format(format_json5);
    let schema = message_format_to_json_schema(&format, name).expect("golden format maps");
...
fn golden_schemas_match_committed_output() {
    ...
    for (name, format_json5, committed) in GOLDENS {
        let rendered = rendered_schema(name, format_json5);
        if update {
            let path = Path::new(env!("CARGO_MANIFEST_DIR")).join(format!(
                "src/generator/exposure/json_schema/goldens/{name}.schema.json"
            ));
        ...
        assert_eq!(
            ...
            "schema for `{name}` drifted from its golden; run \
             `UPDATE_EXPOSURE_GOLDENS=1 cargo test -p generator --lib exposure` and review the diff"
        );
    }
}

fn compiled_validator(name: &str, format_json5: &str) -> jsonschema::Validator {
    let schema: serde_json::Value =
        serde_json::from_str(&rendered_schema(name, format_json5)).expect("schema parses back");
...
fn golden_schemas_judge_canonical_instances() {
    let all_primitives = compiled_validator("all_primitives", golden_format("all_primitives"));
    let full = json!({
        "flag": true,
        "label": "front",
        "blob": "aGVsbG8=",
    ...
    let optionals = compiled_validator("optionals", golden_format("optionals"));
    assert!(
    ...
    let nested = compiled_validator("nested_object", golden_format("nested_object"));
    ...
    let arrays = compiled_validator("arrays", golden_format("arrays"));
    let good = json!({
        "readings": [],
        "corners": [1, 2, 3, 4],
...
fn a_fixed_length_array_of_objects_is_refused() {
    ...
    let err = message_format_to_json_schema(&format, "poses_topic")
        .expect_err("fixed arrays hold scalars only");
    assert!(
        matches!(err, Error::UnsupportedFixedArrayItemType { .. }),
        "{err}"
    );
}
...
fn a_fixed_length_array_of_strings_is_refused() {
    ...
    let err = message_format_to_json_schema(&format, "names_topic")
        .expect_err("fixed arrays hold scalars only");
    assert!(
        matches!(err, Error::UnsupportedFixedArrayItemType { .. }),
        "{err}"
...
fn a_reserved_field_name_is_refused() {
    ...
    let err = message_format_to_json_schema(&format, "reserved_topic")
        .expect_err("reserved names are refused");
    assert!(
        matches!(err, Error::UnauthorizedMessageFieldName { .. }),
        "{err}"
    );
}
