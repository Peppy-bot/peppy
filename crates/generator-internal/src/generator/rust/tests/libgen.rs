...
async fn main() -> Result<()> {{
    ...
    move_arm::emit_feedback(&messenger, [1, 2, 3]).await?;
    println!(\"server emitted feedback [1, 2, 3]\");
...
",
    ...
    assert!(
        subscriber_stdout.contains("goal accepted=true")
            && subscriber_stdout.contains("feedback new_position=[1, 2, 3]")
            && subscriber_stdout
                .contains("result success=true error=completed final_position=[4, 5, 6]"),
    ...
    assert!(
        exposer_stdout.contains("server received goal arm_id=7 desired=[10, 20, 30]")
            && exposer_stdout.contains("server emitted feedback [1, 2, 3]")
