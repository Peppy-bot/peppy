...
/// Adds a `init_node` function to the generated lib that calls `peppylib::setup_node` and check that the `peppy.json5` config file is up to date with its generated functions
/// In order to check that the `peppy.json5` config of the current node is up to date with the generated Rust code, the sha256 generated by the signature of the function (or find a better way?)
